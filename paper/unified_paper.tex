...
We demonstrate that this failure is \textbf{algorithm-invariant}: independent policy-gradient agents converge to suboptimal equilibria regardless of architecture---linear REINFORCE ($\lambda \approx 0.49$, 54\% survival), MLP ($\lambda \approx 0.05$, extreme free-riding, 6\% survival), and MLP Actor-Critic ($\lambda \approx 0.05$, 6\% survival)---none achieving the cooperation level needed for reliable group survival under 30\% Byzantine adversaries.
...
\begin{enumerate}
    ...
    \item[(C3)] \textbf{Algorithm-Invariant Cooperation Failure}: When tipping points introduce irreversible collapse, independent RL agents fail regardless of architecture (Linear, MLP, Actor-Critic)---converging to suboptimal equilibria ($\lambda \approx 0.05$--$0.5$) with $\leq$54\% survival even for the best-case architecture (Section~\ref{sec:nash_trap}).
...
\paragraph{Independent Policy Gradient with Pure Selfish Payoff (IPPO).}
To test whether cooperation failure is an artifact of weak algorithms, we evaluate three levels of policy architecture---all trained independently per agent, maximizing \emph{only} individual payoff $r_i = (E - c_i) + M\sum c_j / N$ with no reward shaping or social priors.
...
\begin{table}[t]
\centering
\caption{Algorithm-invariant cooperation failure: independent IPPO agents converge to suboptimal equilibria regardless of architecture (non-linear PGG, $N{=}20$, Byz=30\%, 5 seeds).}
\label{tab:emergence}
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{Params/agent} & $\boldsymbol{\bar{\lambda}}$ & \textbf{Survival \%} & \textbf{Welfare} \\
\midrule
IPPO Linear & 5 & 0.488 $\pm$ 0.007 & 54.0 & 24.1 \\
IPPO MLP & 161 & 0.048 $\pm$ 0.004 & \textbf{6.0} & 20.4 \\
IPPO MLP + Critic & 322 & 0.048 $\pm$ 0.004 & \textbf{6.0} & 20.4 \\
\midrule
...
\textbf{Result}: Cooperation failure is \textbf{algorithm-invariant} (Table~\ref{tab:emergence}). Linear agents converge to $\lambda \approx 0.49$ with 54\% survival (partial but inadequate), while \emph{stronger} MLP and Actor-Critic agents converge to $\lambda \approx 0.05$---even more extreme free-riding with only 6\% survival.
This is because $M/N = 0.08 < 1$: the marginal return on one's own contribution is individually irrational. More capable agents exploit this structure more aggressively.
Critically, \emph{no architecture achieves the cooperation level needed for reliable survival}: even the best-case linear architecture reaches only 54\% survival, far below the oracle optimum of 100\%.
...
\begin{itemize}
    ...
    To address whether deeper networks can escape the Nash Trap, we conduct a DNN ablation study (Appendix~\ref{app:dnn_ablation}) with 1--3 hidden-layer MLPs (up to 64-32-16 ReLU units).
    \textbf{All architectures converge to $\lambda \approx 0.500$ with 5.3\% survival}---identical to the linear baseline.
    This confirms that the Nash Trap is a \emph{game-theoretic} property of the social dilemma, not a function approximation limitation.
...
\begin{enumerate}
\item \textbf{Claims}: All main claims (Moral Commitment Spectrum, Nash Trap, unconditional commitment necessity) are supported by experiments within the tested PGG environments. We do not claim generality beyond the evaluated settings. [Yes]
\item \textbf{Limitations}: Discussed in Section~\ref{sec:limitations}. Key limitations include full observability assumption, heuristic design of $g(\theta, R)$, and migration rate constraint. [Yes]
...
\item \textbf{Experiments}:
  ...
  \end{enumerate}
\item \textbf{Code and Data}: Anonymized code repository available at \url{https://anonymous.4open.science/r/EthicaAI/}. All experiments are reproducible with provided scripts. [Yes]
