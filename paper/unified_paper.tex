...
We deploy the same meta-ranking agents in the non-linear PGG (Equations~\ref{eq:resource}--\ref{eq:tipping}) with $R_{\text{crit}} = 0.15$, stochastic shocks ($p_{\text{shock}} = 0.05$, $\delta_{\text{shock}} = 0.15$), and 30\% Byzantine adversaries.\footnote{We select 30\% as it exceeds the standard Byzantine fault tolerance threshold ($f < n/3$) and matches common adversarial fractions in the MARL literature~\citep{lerer2017maintaining}. Sensitivity analysis across $\{0\%, 10\%, 30\%, 50\%\}$ confirms qualitative robustness (Appendix~\ref{app:sensitivity}).}
...
\begin{table}[t]
\centering
\caption{Cooperation failure across evaluated algorithms: all tested approaches converge to suboptimal equilibria in the non-linear PGG ($N{=}20$, Byz=30\%, $T$=50 rounds, $R_{\text{crit}}$=0.15, last 30 episodes, 5--10 seeds).\protect\footnotemark}
\label{tab:emergence}
\begin{tabular}{lcccc}
\toprule
...
\midrule
\multicolumn{5}{c}{\textit{MARL baseline implementations$^*$ (10 seeds)}} \\
\midrule
IPPO-style~\citep{schulman2017proximal} & 8,450 & 0.433 $\pm$ 0.009 & 35.7 & 23.6 \\
MAPPO-style~\citep{yu2022surprising} & 8,450$^\dagger$ & 0.367 $\pm$ 0.010 & 22.3 & 23.1 \\
QMIX-style~\citep{rashid2018qmix} & 8,450$^\ddagger$ & 0.176 $\pm$ 0.011 & \textbf{11.3} & 21.5 \\
\midrule
Fixed $\lambda=0.0$ & --- & 0.000 & 0.0 & 20.0 \\
Fixed $\lambda=1.0$ (oracle) & --- & 1.000 & \textbf{100.0} & \textbf{32.0} \\
\bottomrule
\multicolumn{5}{l}{\small $^*$Minimal implementations capturing core algorithmic ideas; see Appendix~\ref{app:baselines}.} \\
\multicolumn{5}{l}{\small $^\dagger$Plus shared critic (4,225 params). $^\ddagger$Plus mixing network.} \\
\end{tabular}
\footnotetext{All algorithms are statistically significantly below the oracle ($p < 0.001$, Mann-Whitney $U$ test, 20 seeds). Pairwise differences: IPPO vs.\ MAPPO survival difference is \emph{not} significant ($p=1.00$); REINFORCE Linear vs.\ IPPO $\lambda$ difference is significant ($p < 0.001$, Cohen's $d = 1.72$). Bootstrap 95\% CIs for survival: REINFORCE Linear [7.2, 11.2]\%, MLP [6.3, 10.5]\%, IPPO [6.3, 10.7]\%, MAPPO [6.3, 10.7]\%.}
\end{table}

\textbf{Result}: Cooperation failure is \textbf{consistent across all evaluated algorithms} (Table~\ref{tab:emergence}).
...
Minimal implementations of MARL algorithms fare no better: IPPO-style with clipped surrogate converges to $\lambda = 0.433 \pm 0.009$ (35.7\% survival), MAPPO-style with shared critic to $\lambda = 0.367 \pm 0.010$ (22.3\%), and QMIX-style to $\lambda = 0.176 \pm 0.011$ (11.3\%).
In our implementation, algorithms with richer information sharing do not improve survival ($p > 0.05$ for IPPO vs.\ MAPPO survival difference, Mann-Whitney $U$ test); the shared critic and centralized Q do not overcome the fundamental $M/N = 0.08 < 1$ free-riding incentive.
...
Result~\ref{res:group_ess} requires $m < m^* = 0.286$ for group-level ESS.
...
\begin{itemize}
    ...
    \item \textbf{Environment Scope}: Primary results are demonstrated in PGG environments with scalar resource state $R_t$.
    As preliminary cross-environment validation, we tested the Moral Commitment Spectrum on a \emph{structurally distinct} Common-Pool Resource (CPR) game (Appendix~\ref{app:cpr}), where agents \emph{extract} from rather than \emph{contribute} to a shared resource (appropriation vs.\ provision). In CPR with 30\% adversaries: selfish RL achieved 9.3\% survival, situational commitment \emph{worsened} to 6.5\% (confirming crisis-mode commitment reduction is counterproductive), while unconditional commitment achieved 46.0\%---the highest survival rate. This provides \emph{initial evidence} that the Spectrum generalizes beyond PGG.
    Validation in spatially-extended benchmarks (Melting Pot~\citep{leibo2021scalable}) remains an important next step.
    Our meta-ranking reward function is architecturally modular and can be integrated with spatial encoders; we leave this as future work.
    \item \textbf{Price Equation Assumptions}: Group-level ESS requires migration rate $m < m^*$ (Result~\ref{res:group_ess}).
    ...
    Appendix~\ref{app:sensitivity} provides sensitivity analysis over $M \in [1.2, 2.0]$ and $f(R_t)$ parameters (12 conditions). The Nash Trap persists across all tested configurations ($\lambda$ range 0.43--0.52, survival 5--58\%).
\end{itemize}
...
In non-linear environments with catastrophic tipping points, pure RL with independent policy gradient fails (Nash Trap: $\lambda \approx 0.5$, $\leq$54\% survival even for the best-case architecture), and only unconditional commitment ($\phi_1^* = 1.0$) reliably prevents collapse across all tested conditions.

This finding suggests a design principle for the tested environment class: \textbf{multi-agent systems operating near irreversible tipping points may need to incorporate moral commitment as a hard constraint, calibrated to the environment's recovery dynamics.}
...
The contribution is not the hard constraint itself---which is trivial to implement---but the systematic, multi-environment demonstration of \emph{when} (non-linear dynamics), \emph{why} (gradient equilibrium in the dynamic survival game), and \emph{to what degree} ($\phi_1^* = 1.0$) such constraints appear necessary in the tested PGG class.
...
\paragraph{Use of AI Assistance.} Large language models assisted with code debugging, English proofreading, and LaTeX formatting. All experimental design, hypothesis formulation, parameter selection, data analysis, and scientific conclusions were conducted by the authors.
