...
Purely selfish REINFORCE agents converge to a \emph{Nash Trap} at $\lambda \approx 0.5$, with survival dropping to 5.3\% under 30\% Byzantine adversaries.
...
\begin{enumerate}
    ...
    \item[(C3)] \textbf{The Nash Trap in Non-linear Environments}: When tipping points introduce irreversible collapse, both situational commitment and pure RL (decentralized REINFORCE) fail---agents converge to $\lambda \approx 0.5$, insufficient to prevent catastrophe (Section~\ref{sec:nash_trap}).
...
\paragraph{Decentralized REINFORCE with Pure Selfish Payoff.}
We train decentralized REINFORCE agents with \emph{only} individual payoff as reward---no reward shaping, no intrinsic motivation, no social priors.
...
\midrule
Selfish REINFORCE & 26.0 & 0.500 & 83.3 & 0.501 \\
Selfish REINFORCE + Byz 30\% & 24.2 & 0.500 & \textbf{5.3} & 0.500 \\
\midrule
...
Critically, this result holds at $N=100$ (Table~\ref{tab:scale}), across 4 network architectures (Appendix~\ref{app:dnn_ablation}), and under K-level opponent anticipation (Appendix~\ref{app:kpg}).
...
\paragraph{K-Level Policy Gradient Comparison.}
To test whether recent opponent-shaping advances can escape the Nash Trap, we implement K-Level Policy Gradients (KPG) with $K \in \{0, 1, 2\}$ recursive anticipation levels (Appendix~\ref{app:kpg}).
...
This confirms that even state-of-the-art opponent-shaping cannot overcome the Nash Trap through gradient-based optimization alone.
