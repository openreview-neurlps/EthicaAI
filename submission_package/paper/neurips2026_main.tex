...
(3) \textbf{Situational Commitment} achieves utilitarian-optimal welfare (147.7) using only local information, outperforming both Exact LOLA (cooperation ceiling 0.209) and Independent PPO (learning paralysis at $\approx$0.5) while being 680$\times$ faster.
We further demonstrate \textbf{scale invariance} (SII$\approx$1.0 up to 1,000 agents), \textbf{robust tracking} under time-varying constraints (Theorem~\ref{thm:tracking}), and \textbf{welfare-optimal design under survival constraints} across computational, cooperation, and adversarial dimensions.
...
\begin{sloppypar}
\textbf{Deep MARL baseline: Independent PPO.}
To address whether modern gradient-based methods can discover cooperation through learning alone, we benchmark against \textbf{Independent PPO (IPPO)} in two reward configurations (Table~\ref{tab:ippo}).
\end{sloppypar}

\begin{table}[h]
\centering
\caption{Meta-Ranking vs.\ IPPO baselines ($N{=}20$ PGG, 5 seeds).}
\label{tab:ippo}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{Coop Rate} & \textbf{Sustain.} & \textbf{Time} \\
\midrule
Meta-Ranking & \textbf{0.576} {\small$\pm$0.039} & 1.000 & 10s \\
Prosocial IPPO & 0.520 {\small$\pm$0.019} & 1.000 & 7,049s \\
Selfish IPPO & 0.518 {\small$\pm$0.019} & 1.000 & 7,073s \\
\bottomrule
\end{tabular}
\end{table}

\begin{sloppypar}
Both IPPO variants converge to cooperation rates near their sigmoid initialization prior ($\approx$0.5), failing to learn purposeful policies despite 5 training episodes. This \textit{learning paralysis} arises from multi-agent non-stationarity: 20 simultaneously adapting agents destabilize each other's value landscapes, preventing credit assignment convergence. Crucially, adding prosocial reward shaping ($w{=}0.5$) provides no benefit over selfish learning ($\Delta$Coop $= +$0.002, $p{>}0.8$), confirming that \textit{static reward engineering cannot substitute for dynamic commitment}. Meta-Ranking achieves 11\% higher cooperation while being \textbf{680$\times$} faster, requiring no gradient computation.
\end{sloppypar}
...
\textbf{Evolutionary stability vs.\ welfare optimality.} In standard PGG where $r/N < 1$, defection is the unique Nash equilibrium, making selfish strategies mathematically inevitable winners in evolutionary competition (Table~3). We transparently report this: Situational Commitment achieves only 0.1\% ESS in well-mixed populations. However, the purpose of meta-ranking is not evolutionary survival of the strategy itself, but \textit{maximizing collective welfare while the strategy is active}. When present, meta-ranking agents match the utilitarian-optimal welfare (147.7) using only local information---without requiring the centralized utility aggregation that pure utilitarians assume---while achieving 0.576 cooperation (vs.\ LOLA's 0.209 and IPPO's 0.518), demonstrating that commitment mechanisms discover socially optimal outcomes that evolution and gradient-based learning alone cannot.

\section{Conclusion}

We provide the first computational verification of Sen's Meta-Ranking theory, demonstrating that \textbf{Situational Commitment}---morality conditional on survival---is the \textbf{welfare-optimal} design principle for moral AI. Our framework scales to 1,000 agents, resists 50\% adversarial populations, and achieves utilitarian-optimal welfare using only local information. Comprehensive baselines confirm meta-ranking's superiority: Exact LOLA suffers computational intractability ($\mathcal{O}(N^3)$, OOM at $N{=}100$) and cooperation failure (best 0.209 vs.\ our 0.576), while Independent PPO exhibits learning paralysis ($\approx$0.5, failing to escape sigmoid initialization) despite being 680$\times$ slower---establishing meta-ranking as a fundamentally superior paradigm for $N$-player moral coordination.
While standard PGG evolutionary dynamics favor defection (a mathematical consequence of $r/N < 1$), extending the framework to resource-depleting environments---where $\lambda_t$'s survival-conditional design becomes advantageous---is a promising direction for future work.
