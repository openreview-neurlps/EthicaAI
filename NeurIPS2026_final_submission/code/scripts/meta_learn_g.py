"""
Track C: Meta-Learning g(胃, R)
Optimize the moral commitment function g_?(胃, R) via MAML-style meta-learning.
...
ENDOWMENT = 20.0
ALPHA_EMA = 0.6           # EMA smoothing for 位_t
...
# ============================================================
# Parameterized g_?(胃, R) ??Learnable Moral Function
# ============================================================
class LearnableG:
    """Parameterized g function with 6 learnable parameters.

    g_?(胃, R) = sigmoid(??�路sin(胃)) 路 clamp(???+ ??偮稲, 0, 1)   if R < ???(crisis)
              = sigmoid(??�路sin(胃)) 路 clamp(???+ ??吢稲, 0, 1)   if R > 1-???(abundance)
              = sigmoid(??�路sin(胃)) 路 (???+ ??偮稲)               otherwise
    ...
    def __init__(self, phi=None):
        if phi is None:
            ...
            self.phi = np.array([
                ...
                0.21,   # ??? base commitment in crisis (??0.3路sin(45掳))
                0.0,    # ??? slope in crisis (hand-crafted: 0)
                0.2,    # ??? crisis threshold R_crisis
                0.75,   # ??? base in abundance (??1.5路sin(45掳))
...
def outer_loss(g_func, seeds, byz_fracs=None):
    """Compute outer loss across multiple tasks (seeds 脳 byz_fracs).

    Loss = -mean(welfare) + 2路mean(resource_variance)
    Lower is better.
    """
...
def meta_learn(n_steps=N_META_STEPS, lr=META_LR, verbose=True):
    ...
    if verbose:
        print("=" * 60)
        print("  Track C: Meta-Learning g_?(胃, R)")
...
def ablation_comparison(learned_g):
    ...
    for bf in byz_fracs:
        ...
        for metric in ["welfare", "cooperation", "resource_stability", "sustainability"]:
            ...
            print(f"  {bf*100:5.0f}% | {metric:>12} | {l_mean:10.3f}卤{np.std(l_vals):.2f} | {h_mean:10.3f}卤{np.std(h_vals):.2f} | {delta:+8.3f}")
