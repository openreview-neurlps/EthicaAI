...
Standard PG:    胃_i ??胃_i + 伪 路 ??{胃_i} V_i(胃)
LOLA:           胃_i ??胃_i + 伪 路 ??{胃_i} V_i(胃 + 伪路?嘨_{-i})
...
class LOLAAgent:
    ...
    Policy: 位 = sigmoid(w 路 obs + b), with Gaussian exploration.
...
def run_lola(seed):
    ...
    for ep in range(N_EPISODES):
        ...
        # For agent i, imagine all j?爄 take a gradient step, then compute
        ...
        for i in range(N_HONEST):
            ...
            for j in range(N_HONEST):
                if j == i:
                    continue
                # Opponent j would update: 胃_j ??胃_j + LR 路 ??j V_j
                # Effect on agent i's gradient (cross-derivative approximation):
                # ?G_i ??d虏V_i/(d胃_i路d胃_j) 路 (LR 路 ??j V_j)
