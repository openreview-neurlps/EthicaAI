"""
Independent REINFORCE Nash Trap Experiment (v3)
=================================================
...
This script implements INDEPENDENT policy gradient (REINFORCE):
  - Each agent has its own policy parameters
  - Each agent maximizes its own payoff r_i = (E - c_i) + M*sum(c)/N
  - The individually rational strategy is to free-ride (low lambda)
...
All should converge to lambda ~ 0.3-0.6 (Nash equilibrium).

NOTE: Despite the historical filename (ppo_nash_trap.py), this script
implements independent REINFORCE, NOT PPO. The wrapper stub
ppo_nash_trap.py redirects here for backward compatibility.
        ...
        for ep in range(n_ep):
            ...
            for i in range(n_honest):
                ...
                if isinstance(agents[i], LinearAgent):
                    ...
                    # Gaussian REINFORCE for MLP: grad log N(a|mu(s),sigma) * G_t
                    # mu = sigmoid(MLP(obs)), sigma fixed = 0.15
                    sigma = 0.15
                    lr = agents[i].lr
                    ...
                    for t_idx in range(len(returns)):
                        ...
                        if isinstance(agents[i], MLPCriticAgent):
                            ...
                            advantage = G_t - v_t
                            # Update critic: V <- V + lr_v * (G - V) * grad
                        ...
                        # d(logit)/d(W2) = h, d(logit)/d(b2) = 1
                        agents[i].W2 += lr * delta * h.reshape(-1, 1)
                        agents[i].b2 += lr * delta
                        
                        # d(logit)/d(h) = W2.flatten()
                        # d(h)/d(z1) = 1 - tanh^2(z1) (tanh derivative)
