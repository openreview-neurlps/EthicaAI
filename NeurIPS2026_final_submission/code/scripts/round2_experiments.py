...
================================================
Phase A: HP Sweep (IPPO lr Ã— entropy, 12 combos Ã— 3 seeds)
Phase B: Commitment Ablation (5 variants Ã— 3 seeds)
...
def phase_a():
    ...
    for lr in LR_GRID:
        for ent in ENT_GRID:
            ...
            print(f"  lr={lr:.0e} ent={ent:.2f}: Î»={np.mean(lams):.3f} surv={np.mean(survs):.1f}% {st}")
...
def phase_b():
    ...
    labels = {
        "full": "Full g(Î¸,R)", "no_crisis": "No crisis zone",
        "no_abundance": "No abundance bonus", "alpha_zero": "No smoothing (Î±=0)",
        "alpha_one": "No adaptation (Î±=1)", "no_restraint": "No restraint (Î²=0)",
    }
    ...
    for var in variants:
        ...
        print(f"  {labels[var]:>25s}: Î»={np.mean(lams):.3f}Â±{np.std(lams):.3f} surv={np.mean(survs):.1f}Â±{np.std(survs):.1f}% W={np.mean(wels):.1f}Â±{np.std(wels):.1f}")
...
def phase_d():
    ...
    for N in [2, 5, 10, 20, 50, 100]:
        ...
        # 1st order gradient: ?‚r_i/?‚Î?i
        grad1 = -1.0 + M/N * p_surv + M/N * np.sum(lambdas) * dp/N
        
        # 2nd order cross-derivative: ?‚Â²r_i/?‚Î?i?‚Î?j
        d2p = dp * (1 - 2*p_surv) * 20 / N
        cross = M/N * (dp/N + np.sum(lambdas) * d2p/N)
        
        # LOLA correction magnitude: Î£_{j? i} lr_opp * grad_j * cross
        lr_opp = 0.01
    ...
    print(f"\n  N=20 LOLA trajectory ??Î»_final = {trajectory[-1]:.4f}")
