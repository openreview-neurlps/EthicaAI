...
This script implements INDEPENDENT policy gradient:
  ...
  - Each agent maximizes its own payoff r_i = (E - c_i) + M路sum(c)/N
  - The individually rational strategy is to free-ride (low 位)
  - But collective free-riding leads to resource collapse

We test THREE architectures:
  1. Linear (sigmoid(w路obs + b)) ??5 params per agent
  2. MLP (2-layer 32-unit) ??~161 params per agent
  3. MLP + Value baseline ??~322 params per agent

All should converge to 位??.3~0.6 (Nash equilibrium).
        ...
        for ep in range(n_ep):
            ...
            for i in range(n_honest):
                ...
                if isinstance(agents[i], LinearAgent):
                    ...
                    # Gaussian REINFORCE for MLP: ?囄?log N(a|渭(s),?) 路 G_t
                    # 渭 = sigmoid(MLP(obs)), ? fixed = 0.15
                    sigma = 0.15
                    lr = agents[i].lr
                    ...
                    for t_idx in range(len(returns)):
                        ...
                        if isinstance(agents[i], MLPCriticAgent):
                            ...
                            advantage = G_t - v_t
                            # Update critic: V ??V + lr_v 路 (G - V) 路 ?嘨
                        ...
                        # Forward: 渭 = sigmoid(W2 路 tanh(W1路obs + b1) + b2)
