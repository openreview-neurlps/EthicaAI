...
We demonstrate that this failure is \textbf{consistent across seven evaluated paradigms}: independent policy-gradient (REINFORCE) agents across three architectures---linear ($\lambda \approx 0.49$, 54\% survival), MLP ($\lambda \approx 0.37$, 31\% survival), and MLP Actor-Critic ($\lambda \approx 0.37$, 26\% survival)---as well as PPO (IPPO/MAPPO), independent Q-learning (IQL), \textbf{value decomposition} (QMIX with monotonic mixing network, 67\% survival; Appendix~\ref{app:qmix_lola}), and \textbf{opponent-shaping} (LOLA, 51\% survival; Appendix~\ref{app:qmix_lola}), none achieving reliable group survival under 30\% Byzantine adversaries.
The commitment floor $\phi_1$ acts as a policy lower bound on cooperation: agents trained with $\phi_1{=}1.0$ achieve 100\% survival under 30\% Byzantine adversaries, while $\phi_1{=}0$ (pure learning) results in only 30\% survival.
...
\begin{enumerate}
    \item[(C1)] \textbf{Operationalization of Sen's Meta-Ranking}: We formalize commitment as a dynamic parameter $\lambda_t \in [0,1]$ that adapts to resource state $R_t$ and Social Value Orientation $\theta_i$. We acknowledge this is a \emph{simplified operationalization}: $\lambda$ linearly interpolates between self-interest and prosociality, rather than implementing the full deliberative preference-over-preferences that Sen envisioned. Nevertheless, this operationalization is sufficient to reveal the core phenomenon (Section~\ref{sec:method}).
    ...
    \item[(C4)] \textbf{Unconditional Commitment in Non-linear PGG}: In our tested environment class, only maximal crisis commitment ($\phi_1^* = 1.0$) reliably prevents collapse. Meta-learning independently recovers this optimum, providing convergent evidence within this setting (Section~\ref{sec:unconditional}).
\end{enumerate}

Together, these results provide evidence for a \textbf{Moral Commitment Spectrum} within our tested PGG family: the severity of environmental non-linearity determines the minimum commitment required for collective survival in this class of social dilemmas.
...
We operationalize unconditional commitment as a \emph{policy floor}: for each agent $i$, the effective cooperation level is $\lambda_i^{\text{eff}} = \max(\lambda_i^{\text{learned}}, \phi_1)$, where $\lambda_i^{\text{learned}}$ is the output of the learned policy and $\phi_1 \in [0, 1]$ is a system-level design parameter.
This is the precise operationalization of Sen's ``unconditional commitment'': $\phi_1 = 0$ permits pure self-interest, while $\phi_1 = 1$ enforces full cooperation regardless of the learned policy.
...
\begin{itemize}
    ...
    \item \textbf{Philosophical Fidelity}: Our $\lambda$-based model is a \emph{simplified operationalization} of Sen's meta-ranking, not a faithful computational implementation of the full theory. True meta-ranking involves deliberative ranking over alternative preference orderings, whereas our model uses dynamic linear interpolation between self-regarding and other-regarding rewards. Developing richer meta-ranking architectures---e.g., discrete preference selection or learned ranking networks---is an important direction for future work.
