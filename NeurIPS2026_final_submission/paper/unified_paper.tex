...
\textbf{Algorithm scope}: we test 9 implementations across 5 algorithm families plus CleanRL IPPO/MAPPO (4,546 params); all fall into the Nash Trap. CTDE with global-state critics and communication channels are tested (Appendix~\ref{app:comm_ctde_neural}): 100\% trap rate.
...
We identify the \textbf{Nash Trap}---a locally stable, collectively catastrophic equilibrium in Tipping-Point Social Dilemmas---that persists across 11 algorithm variants (including CleanRL IPPO/MAPPO with 4,546-parameter MLPs), $400{+}$ hyperparameter configurations, and multiple environment classes.
