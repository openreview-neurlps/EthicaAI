...
\midrule
IPPO (CleanRL-style)~\citep{schulman2017proximal} & 4,546 & 0.401 $\pm$ 0.063 & 37.3 & 23.4 \\
MAPPO (CleanRL-style)~\citep{yu2022surprising} & 4,546$^\dagger$ & 0.392 $\pm$ 0.050 & 36.2 & 23.3 \\
...
\bottomrule
\multicolumn{5}{l}{\small CleanRL-style: single-file NumPy reimplementations following CleanRL~\citep{huang2022cleanrl} conventions.} \\
\multicolumn{5}{l}{\small HP sweep (20 combos $\times$ 10 seeds = 200 runs) in Appendix~\ref{app:hp_sweep}.} \\
...
\textbf{Result}: The Nash Trap presents a significant barrier across the evaluated algorithms, though to varying degrees (Table~\ref{tab:emergence}).
...
Standard MARL implementations (CleanRL-style) spanning three paradigms---independent policy gradient (IPPO), centralized critic (MAPPO), and value decomposition (QMIX)---all converge to suboptimal equilibria.
...
IPPO converges to $\lambda = 0.401 \pm 0.063$ (37.3\% survival, 20 seeds), and MAPPO to $\lambda = 0.392 \pm 0.050$ (36.2\%).
Notably, QMIX's value decomposition yields significantly higher cooperation ($\lambda = 0.584 \pm 0.021$, 71.8\% survival). This demonstrates that global value factorization mitigates the cooperation failure compared to independent policy gradients. However, even QMIX falls \emph{substantially} short of the oracle ($\lambda = 1.0$, 100\% survival) and unconditional priors, suggesting that the Nash Trap remains a robust structural barrier in the tested non-linear PGG environment.
This result holds at $N=100$ (Table~\ref{tab:scale}), under K-level opponent anticipation ablations (Appendix~\ref{app:kpg}), across a hyperparameter sweep (Table~\ref{tab:hp_sweep}, 20 combinations $\times$ 10 seeds = 200 runs, Appendix~\ref{app:hp_sweep}), commitment function ablations (Table~\ref{tab:ablation}), CVaR risk-sensitive objectives (Table~\ref{tab:cvar}), partial observability (Table~\ref{tab:partial_obs}), adaptive adversaries (Table~\ref{tab:adaptive_adv}), and cross-environment CPR validation (Table~\ref{tab:cpr}). Training dynamics and the Nash Trap cost landscape are visualized in Figures~\ref{fig:training_curves} and~\ref{fig:nash_trap}; multi-axis normalized comparison appears in Table~\ref{tab:melting_pot} and Figure~\ref{fig:melting_pot} (see Appendix~\ref{app:robustness}).
...
\midrule
0.00 & 25.7 $\pm$ 0.6 & 24.7 $\pm$ 0.3 & 29 $\pm$ 10\% & 19 $\pm$ 6\% \\
0.21 & 25.8 $\pm$ 0.4 & 24.8 $\pm$ 0.3 & 34 $\pm$ 10\% & 22 $\pm$ 7\% \\
0.50 & 25.6 $\pm$ 0.2 & 25.2 $\pm$ 0.2 & 82 $\pm$ 9\% & 42 $\pm$ 9\% \\
\textbf{1.00} & \textbf{30.8 $\pm$ 0.0} & \textbf{27.5 $\pm$ 0.0} & \textbf{100 $\pm$ 0\%} & \textbf{96 $\pm$ 4\%} \\
\bottomrule
\end{tabular}
\end{table}
...
Our robustness experiments (Appendix~\ref{app:robustness}) include adaptive adversary scenarios that partially address this---showing unconditional commitment retains structural advantages even under adversarial pressure (100.0\% vs.\ 11.4--39.0\% survival for selfish RL across three adversary types, Table~\ref{tab:adaptive_adv})---but real-world deployment requires additional safeguards.
...
\caption{Criteria-inspired profile comparison (\textbf{qualitative}). Scores for our method and IPPO/MAPPO/QMIX are derived from PGG/CPR experiment JSON outputs; A3C+SVO and Inequity Averse scores are estimated from published results. This figure serves as a high-level overview; see individual tables for rigorous quantitative comparisons. Meta-Ranking ranks 1st in 4/5 dimensions within the tested environments.}
