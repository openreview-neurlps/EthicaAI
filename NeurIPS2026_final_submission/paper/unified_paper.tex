...
Furthermore, neither cheap-talk communication, centralized critics (CTDE), nor neural network policies (97-parameter MLPs) escape the trap (Appendix~\ref{app:comm_ctde_neural}): all three achieve 100\% trap rate across 20 seeds.
...
\noindent \textbf{Key finding}: the Nash Trap persists under (1)~cheap-talk communication, where agents broadcast a 1D message before acting; (2)~a shared centralized critic that observes the global state $[R_t, \bar\lambda_t]$; and (3)~2-layer MLP policies with 97 learned parameters per agent. In all cases, $\bar\lambda \approx 0.5$ with 100\% trap rate. These results are consistent with our theoretical analysis (Proposition~\ref{prop:gradient_impossibility}): the trap arises from the gradient landscape structure, not from limited agent capacity or information.
