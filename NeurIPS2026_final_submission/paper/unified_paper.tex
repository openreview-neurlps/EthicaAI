...
Constrained MDPs such as CPO~\citep{achiam2017constrained} and risk-sensitive objectives (CVaR) provide safety guarantees by constraining reward optimization~\citep{garcia2015comprehensive}.
...
Our contribution extends this line by identifying \emph{when} each moral commitment level is necessary: the Moral Commitment Spectrum provides a principled mapping from environmental non-linearity to the required degree of commitment.
...
\midrule
CleanRL IPPO~\citep{schulman2017proximal} & 4,546 & 0.401 $\pm$ 0.063 & 37.3 & 23.4 \\
CleanRL MAPPO~\citep{yu2022surprising} & 4,546$^\dagger$ & 0.392 $\pm$ 0.050 & 36.2 & 23.3 \\
QMIX~\citep{rashid2018qmix} & 5,195$^\ddagger$ & 0.584 $\pm$ 0.021 & 71.8 & 24.9 \\
\midrule
...
\end{tabular}}
\footnotetext{All experiments use $E{=}20$ (endowment). All algorithms significantly below oracle ($p < 0.001$, Mann-Whitney $U$, 20 seeds). 95\% CIs: IPPO $\lambda$ [0.374, 0.429]; MAPPO $\lambda$ [0.370, 0.414]; QMIX $\lambda$ [0.575, 0.593]. Wall-clock: 18--45s/seed (CPU).}
\end{table}
...
Crucially, this is not a failure of learning per se: the algorithms \emph{successfully} converge to a local optimum of their reward landscape. The Nash Trap is a \textbf{structural outcome} of individually-rational optimization in a game where the collective benefit of cooperation is not captured by individual payoffs.
IPPO converges to $\lambda = 0.401 \pm 0.063$ (37.3\% survival, 20 seeds), MAPPO to $\lambda = 0.392 \pm 0.050$ (36.2\%), and QMIX to $\lambda = 0.584 \pm 0.021$ (71.8\%).
...
\midrule
0.00 & 27.3 $\pm$ 0.2 & 22.6 $\pm$ 0.4 & 59 $\pm$ 12\% & 22 $\pm$ 6\% \\
0.21 & 29.1 $\pm$ 0.1 & 24.0 $\pm$ 0.2 & 71 $\pm$ 9\% & 34 $\pm$ 9\% \\
0.50 & 30.8 $\pm$ 0.1 & 25.7 $\pm$ 0.2 & 83 $\pm$ 10\% & 50 $\pm$ 10\% \\
\textbf{1.00} & \textbf{32.0 $\pm$ 0.0} & \textbf{28.2 $\pm$ 0.1} & \textbf{100 $\pm$ 0\%} & \textbf{91 $\pm$ 8\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Algorithm Scope}: Whether risk-sensitive objectives or dynamic externality-pricing mechanisms can escape the Trap remains open. While empirical CVaR objectives fail (Appendix~\ref{app:robustness}), Constrained Policy Optimization (CPO) with parameterized safety constraints on $R_t$ offers a promising future direction.
    \item \textbf{Environment Scope}: Primary results use PGG with scalar $R_t$. CPR validation (Appendix~\ref{app:cpr}) confirms the pattern. Extending to pixel-based spatial environments, specifically \emph{Prisoners' Dilemma in the Matrix} and \emph{Clean Up} subtasks in Melting Pot~\citep{leibo2021scalable}, remains future work. Group-level ESS requires $m < m^* = 0.286$ (Result~\ref{res:group_ess}).
\end{itemize}

\noindent Additional robustness checks (Appendix~\ref{app:robustness}): CVaR objectives, partial observability (noisy/delayed/local $R_t$), and adaptive adversaries all confirm the Nash Trap's persistence.
...
We establish the \textbf{Moral Commitment Spectrum}: in linear environments, situational commitment achieves group-level ESS; in non-linear environments with tipping points, only unconditional commitment ($\phi_1^* = 1.0$) prevents collapse \emph{across all tested algorithms and environment configurations}.
This is supported by the Nash Trap ($\lambda{\approx}0.5$), KPG failure (Appendix~\ref{app:kpg}), and spatial replication (Appendix~\ref{app:spatial}).
...
Our Spectrum implies a concrete design principle for AI safety: for autonomous systems operating near irreversible environmental tipping points, cooperative constraints must be structurally unconditional ($\phi_1=1.0$), as adaptive or situational moral priors will systematically collapse under selfish gradient pressure.
...
Unconditional commitment strategies, if adopted asymmetrically, could be exploited by adversaries: e.g., a cooperative agent that always contributes maximally may subsidize free-riders without recourse.
In deployed systems, commitment mechanisms should be paired with monitoring and accountability structures.
Our robustness experiments (Appendix~\ref{app:robustness}) include adaptive adversary scenarios that partially address this---showing unconditional commitment retains structural advantages even under adversarial pressure (73.0\% vs.\ 26.7\% survival)---but real-world deployment requires additional safeguards.
...
Table~\ref{tab:partial_obs} shows that the Nash Trap persists across all partial observability conditions. Situational commitment performance degrades further under noisy or delayed signals, while unconditional commitment remains impervious, as it requires no environmental feedback.
...
\midrule
Full Obs (Baseline) & X\% & X\% & X\% \\
Noisy ($\sigma=0.10$) & X\% & X\% & X\% \\
Noisy ($\sigma=0.20$) & X\% & X\% & X\% \\
Delayed ($k=2$) & X\% & X\% & X\% \\
Delayed ($k=5$) & X\% & X\% & X\% \\
Local-only & X\% & X\% & X\% \\
\bottomrule
...
\noindent All scores can be exactly reproduced from our experiment logs using the provided \texttt{compute\_radar\_scores.py} script.
...
\noindent All three paradigms---independent PG (IPPO, $\lambda{=}0.401$), centralized critic (MAPPO, $\lambda{=}0.392$), and value decomposition (QMIX, $\lambda{=}0.584$)---converge below the oracle.
...
\begin{table}[h]
\centering
\caption{Hyperparameter sweep: Nash Trap persists across all 12 lr$\times$entropy combinations (non-linear PGG, $N{=}20$, $E{=}20$, Byz=30\%, 150 episodes, 3 seeds).}
\label{tab:hp_sweep}
\resizebox{\textwidth}{!}{\begin{tabular}{cccccc}
\toprule
\textbf{Learning Rate} & \textbf{Entropy} & $\boldsymbol{\bar{\lambda}}$ & \textbf{Surv.\%} & \textbf{Trapped?} \\
\midrule
$10^{-4}$ & 0.00 & 0.428 & 48.3 & Yes \\
$10^{-4}$ & 0.01 & 0.428 & 48.3 & Yes \\
$10^{-4}$ & 0.05 & 0.428 & 48.3 & Yes \\
$2.5{\times}10^{-4}$ & 0.00 & 0.431 & 43.3 & Yes \\
$2.5{\times}10^{-4}$ & 0.01 & 0.431 & 43.3 & Yes \\
$2.5{\times}10^{-4}$ & 0.05 & 0.431 & 43.3 & Yes \\
$5{\times}10^{-4}$ & 0.00 & 0.404 & 41.7 & Yes \\
$5{\times}10^{-4}$ & 0.01 & 0.404 & 41.7 & Yes \\
$5{\times}10^{-4}$ & 0.05 & 0.404 & 41.7 & Yes \\
$10^{-3}$ & 0.00 & 0.420 & 38.3 & Yes \\
$10^{-3}$ & 0.01 & 0.420 & 38.3 & Yes \\
$10^{-3}$ & 0.05 & 0.420 & 38.3 & Yes \\
\bottomrule
\end{tabular}}
\end{table}

\noindent All 12 combinations converge to $\lambda \in [0.404, 0.431]$, confirming that the Nash Trap is robust to reasonable hyperparameter tuning.
...
\item \textbf{Experiments}:
  \begin{enumerate}
    ...
    \item Seeds: 20 per experiment (standardized). [Yes]
