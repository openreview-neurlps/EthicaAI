...
\multicolumn{5}{l}{\small $^\|$Per-agent Q + mixing net. Canonical PyTorch QMIX confirms results (Appendix~\ref{app:canonical_qmix}).} \\
\end{tabular}}
\footnotetext{$E{=}20$. Mann-Whitney $U$ with Holm--Bonferroni correction ($K{=}5$ vs.\ oracle): Cliff's $|\delta|{=}1.0$ for all pairs (maximum separation). 95\% CIs (20 seeds): IPPO $\lambda$ [0.368, 0.422]; MAPPO [0.362, 0.424]; IQL [0.575, 0.593]. Wall-clock: 18--198s/seed (CPU). Full output: \texttt{compute\_statistics.py}.}
\end{table}
