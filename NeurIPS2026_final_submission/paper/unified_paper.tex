...
% ================================================================
\begin{abstract}
When must multi-agent systems move beyond self-interest, and how much commitment is enough for collective survival? We study this question in \textbf{Tipping-Point Social Dilemmas} (TPSDs), where thresholded resource dynamics can trigger irreversible collapse, and identify an extinction-type failure mode we term the \textbf{Nash Trap}: agents converge to locally stable under-commitment equilibria that are individually rational yet collectively catastrophic.

We make four contributions. (C1)~\textbf{Nash Trap Analysis}: We show that $M/N < 1$ creates a persistent local anti-commitment incentive, and that in environments with tipping-point collapse dynamics this misalignment induces the Nash Trap. Across five algorithm families (REINFORCE, PPO, IQL, QMIX, LOLA; seven implementations), all tested methods converge to subcritical commitment levels under a unified fair-comparison protocol with $400{+}$ hyperparameter configurations and 100\% trap rate \emph{in the tested TPSD settings}. (C2)~\textbf{Commitment Floor as Safety Specification}: We derive the required commitment floor from recovery dynamics near collapse. In severe regimes, the required floor exceeds the feasible range and therefore saturates at unconditional commitment, $\phi_1=1.0$; in milder regimes, partial floors suffice. A Lagrangian dual CPO formulation independently converges to the same specification. (C3)~\textbf{Adaptive Commitment Learning (ACL)}: We introduce a two-phase method that learns a state-dependent floor $\phi_1(R_t;\omega)$, recovering near-unconditional commitment in severe TPSDs while improving welfare by up to 241\% over fixed $\phi_1=1.0$ in milder environments without sacrificing survival. (C4)~\textbf{Cross-Environment Validation and Boundary Tests}: We validate the framework across five environment classes, adversarial conditions (30\% Byzantine), scale ($N=100$), and 36 shock regimes, and show that environments lacking the ingredients of the Trap do not show a commitment-floor advantage in our tests.

Together, these results establish a \textbf{Moral Commitment Spectrum}---a diagnostic mapping from environmental severity to the minimum commitment floor required for collective survival (\emph{not} a monotonic dominance ordering among strategies). Local incentive misalignment explains why commitment is disfavored, tipping-point dynamics explain when under-commitment becomes catastrophic, and recovery severity determines how much commitment is required for escape.
\end{abstract}
...
Building on this decomposition, we make four contributions:
\begin{enumerate}
    \item[(C1)] \textbf{Nash Trap Analysis}: We define the class of Tipping-Point Social Dilemmas (Definition~\ref{def:tpsd}), prove a drift condition on resource dynamics (Lemma~\ref{lem:drift}), derive finite-horizon survival bounds (Lemma~\ref{lem:survival}), and establish the Nash Trap as a locally stable fixed point (Proposition~\ref{prop:nash_trap}; see also Remark~\ref{rem:nash_trap_vs_ne} for disambiguation from static Nash equilibria). The mechanism has three layers: (i)~$M/N < 1$ creates a persistent local anti-commitment incentive (Proposition~\ref{prop:gradient_impossibility}); (ii)~tipping-point dynamics convert this under-commitment into irreversible collapse; (iii)~recovery asymmetry $\rho$ determines the commitment floor required for escape. Full proofs are in Appendix~\ref{app:theory}.
    We demonstrate that this failure is \emph{consistent across five algorithm families} (REINFORCE, PPO, IQL, QMIX~\citep{rashid2018qmix}, and LOLA~\citep{foerster2018learning}; seven implementations)---all converging to $\lambda \approx 0.5$ ($\leq$72\% survival) under a unified fair comparison protocol with $400{+}$ HP configurations (Section~\ref{sec:nash_trap}). A canonical QMIX re-implementation following EPyMARL~\citep{papoudakis2021benchmarking} conventions confirms these results (Appendix~\ref{app:canonical_qmix}, Table~\ref{tab:canonical_qmix}).
    \item[(C2)] \textbf{Commitment Floor as Safety Specification}: The required commitment floor is derived from the environment's recovery function near its tipping point (Theorem~\ref{thm:critical}). In severe regimes the required floor exceeds the feasible range and saturates at $\phi_1 = 1.0$ (Corollary~\ref{cor:unconditional}); this is independently confirmed via Lagrangian dual CPO convergence (Appendix~\ref{app:cpo_lagrangian}, Table~\ref{tab:cpo_lagrangian}) and grid search across 110 conditions (Section~\ref{sec:unconditional}). Phase diagrams visualize the Moral Commitment Spectrum.
    \item[(C3)] \textbf{Adaptive Commitment Learning (ACL)}: While $\phi_1{=}1.0$ is \emph{survival-optimal}, it can be \emph{welfare-suboptimal}. We introduce ACL, a two-phase algorithm that learns a state-dependent floor $\phi_1(R_t; \omega) = \sigma(\omega_1 R_t + \omega_2 R_t^2 + \omega_3)$ via safety-constrained welfare optimization. In Cleanup-style environments, ACL improves welfare by $241\%$ over fixed $\phi_1{=}1.0$ while maintaining $100\%$ survival (Table~\ref{tab:acl}); in severe TPSDs, it automatically recovers $\phi_1 \approx 1.0$ (Appendix~\ref{app:acl}).
    \item[(C4)] \textbf{Cross-Environment Validation and Boundary Tests}: We validate the Nash Trap and commitment floor across five environment classes sharing incentive misalignment and thresholded collapse---PGG, CPR (Appendix~\ref{app:cpr}), spatial grids (Appendix~\ref{app:spatial_dilemma}), Cleanup-style commons (Appendix~\ref{app:cleanup}, Table~\ref{tab:cleanup}), and the Coin Game~\citep{lerer2017maintaining} (Appendix~\ref{app:coin_game}, Table~\ref{tab:coin_game})---under adversarial conditions (30\% Byzantine), scale ($N{=}100$, Table~\ref{tab:scale}), and 36 shock regimes. Environments lacking these ingredients do not show a commitment-floor advantage in our tests (Section~\ref{sec:linear}; Appendix~\ref{app:meltingpot_prelim}). A parameter sweep across 60 configurations confirms $\hat{\lambda} < 0.95$ universally (Appendix~\ref{app:nash_bound}).
\end{enumerate}

Together, these results establish a \textbf{Moral Commitment Spectrum}---a diagnostic mapping from environmental severity to the minimum commitment floor required for collective survival. Local incentive misalignment explains \emph{why} commitment is disfavored, tipping-point dynamics explain \emph{when} under-commitment becomes catastrophic, and recovery severity determines \emph{how much} commitment is required for escape. The Spectrum classifies crisis-response profiles rather than ranking them monotonically: in collapse-dominated regimes (e.g., CPR), a context-sensitive profile may underperform selfish baselines because its crisis response reduces restraint exactly when maximal restraint is required (Appendix~\ref{app:cpr}).
...
\footnotetext{$E{=}20$. Mann-Whitney $U$ with Holm--Bonferroni correction ($K{=}5$ vs.\ oracle): Cliff's $|\delta|{=}1.0$ for all pairs (maximum separation). 95\% CIs (20 seeds): IPPO $\lambda$ [0.368, 0.422]; MAPPO [0.213, 0.236]; IQL [0.575, 0.593]. QMIX/LOLA: 2 seeds (CI not reported). Source: \texttt{cleanrl\_baseline\_results.json}, \texttt{iql\_baseline\_results.json}, \texttt{qmix\_baseline\_results.json}, \texttt{lola\_results.json}.}
\end{table}

\textbf{Result}: Across \textbf{five algorithm families} (seven implementations; Table~\ref{tab:emergence}), all tested algorithms converge to suboptimal cooperation levels in our non-linear PGG.
...
IPPO converges to $\lambda = 0.393 \pm 0.062$ (19.0\% survival, 20 seeds), MAPPO to $\lambda = 0.223 \pm 0.027$ (12.2\%), and IQL achieves the highest $\lambda = 0.584 \pm 0.021$ (71.8\% survival) due to its discrete action space and $\varepsilon$-greedy exploration.
Notably, \textbf{QMIX with a monotonic mixing network} ($\lambda = 0.511 \pm 0.005$, 80.0\% survival, 2 seeds; canonical cross-validation at $N{=}5$ with 20 seeds yields $\lambda{=}0.504$, $68.6\%$ survival; Appendix~\ref{app:canonical_qmix}) does not outperform IQL, indicating that value decomposition does not help escape the Nash Trap.
Similarly, \textbf{LOLA} ($\lambda = 0.498 \pm 0.001$, 55.0\% survival, 2 seeds due to $O(N^2)$ scaling) shows that anticipating opponent learning steps is insufficient when the underlying game structure traps all agents in a local equilibrium. Our $N$-agent LOLA uses a simplified 1-step approximation where each agent $i$ estimates the effect of opponents' parameter updates on its own policy gradient using the same trajectory (fixed returns); this implies the LOLA correction captures policy-space effects but not environment-dynamics effects, providing a conservative test.
...
\begin{enumerate}
    \item \textbf{Incentive misalignment} ($M/N < 1$): The marginal return from the public good is below the individual cost of cooperation, so each agent's local payoff gradient pushes commitment downward (Proposition~\ref{prop:gradient_impossibility}). This condition is necessary for the Nash Trap.
    \item \textbf{Trap manifestation} (tipping-point dynamics): Non-linear resource dynamics with collapse thresholds convert this under-commitment into irreversible extinction. Without tipping points, $M/N < 1$ produces suboptimal cooperation but not catastrophic collapse (our linear PGG results, Section~\ref{sec:linear}).
    \item \textbf{Floor severity} ($\rho = f_{\text{norm}}/f_{\text{crit}}$): Recovery asymmetry determines the commitment floor required for escape. High $\rho$ (slow recovery near collapse) requires a higher floor; in the extreme regime ($\rho \geq 10$), the required floor exceeds the feasible range and saturates at $\phi_1 = 1.0$ (Theorem~\ref{thm:critical}). Our 50$\times$ recovery sweep confirms the Trap persists even at $\rho = 0.2$, though the required floor is lower (Table~\ref{tab:recovery_boundary}).
\end{enumerate}

Table~\ref{tab:boundary_tests} summarizes the diagnostic logic of our framework. The key distinction is threefold: local incentive misalignment explains why commitment is disfavored, tipping dynamics explain when under-commitment becomes an extinction-type failure mode, and recovery severity determines how large the commitment floor must be to escape that failure. The observed outcomes across our boundary tests match these predictions.
...
& All six tested approaches remain trapped; unconditional commitment achieves $100\%$ survival \\
...
Unlike opponent-shaping methods (LOLA, M-FOS, KPG) that learn to influence others' learning via meta-gradients, meta-ranking implements pre-specified moral structure without opponent modeling. The Nash Trap, LOLA failure ($\lambda = 0.490$, 50.8\% survival), QMIX failure ($\lambda = 0.524$, 66.5\%), KPG failure (Appendix~\ref{app:kpg}), IA/SI collapse (Table~\ref{tab:baselines}), and \emph{team reward shaping} failure ($\lambda = 0.597$, 72.0\% survival; Table~\ref{tab:team_reward}) collectively demonstrate that moral commitment was not learned by any of the \textbf{six tested approaches} (five algorithm families plus reward shaping) in our non-linear PGG environments---confirming that structural commitment constraints are necessary for this class of problems.
...
\subsection{Adaptive Commitment Learning (ACL)}
...
This welfare loss is the \emph{cost of safety}: unconditional commitment sacrifices individual optimality to guarantee collective survival.
A natural question is whether this cost is avoidable when the environment permits it.

\paragraph{Algorithm.}
We introduce \textbf{Adaptive Commitment Learning (ACL)}, a two-phase algorithm:
\begin{enumerate}
    \item \textbf{Phase 1 (Safety Discovery)}: Train IPPO with $\phi_1{=}1.0$ for $T_1$ episodes to establish a survival baseline. Record state trajectories $\{R_t\}$.
    \item \textbf{Phase 2 (Welfare Optimization)}: Learn a state-dependent floor $\phi_1(R_t; \omega) = \sigma(\omega_1 R_t + \omega_2 R_t^2 + \omega_3)$ via Evolution Strategy (ES), maximizing $W(\omega) - \mu \cdot \max(0, \delta - P_{\text{surv}}(\omega))$ where $\mu$ is a Lagrangian penalty and $\delta{=}0.95$ is the survival constraint.
\end{enumerate}
\noindent The transition criterion is $P_{\text{surv}} \geq 0.95$ over 20 consecutive evaluation episodes.

\paragraph{Results.}
In severe TPSDs (non-linear PGG with $f(R_{\text{crit}}){=}0.01$), ACL autonomously recovers $\phi_1 \approx 1.0$---confirming that Theorem~\ref{thm:critical}'s specification is learnable.
In moderate environments (Cleanup-style commons with $f(R_{\text{crit}}){=}0.05$), ACL discovers dynamic floors:
$\phi_1(R_t) \approx 0.3$ when $R_t > 0.5$ (low crisis), ramping to $\phi_1 \approx 0.9$ as $R_t \to R_{\text{crit}}$.
This improves welfare by up to \textbf{241\%} over fixed $\phi_1{=}1.0$ while maintaining 100\% survival, resolving the survival--welfare tradeoff.
Full details, convergence curves, and ablations are in Appendix~\ref{app:acl}.
...
Empirically, this pattern is robust across five algorithm families, six tested approaches including reward shaping, $400{+}$ hyperparameter configurations, adversarial settings, scale, long horizons, and cross-environment tests. Neither value decomposition, opponent shaping, reward shaping, nor longer horizons reliably escape the Trap under selfish learning. By contrast, commitment floors succeed because they modify the feasible action set rather than relying on misaligned local incentives to discover survival-preserving behavior. ACL further shows that this need not be an all-or-nothing prescription: when environments permit partial commitment, the floor can be learned adaptively to preserve survival while substantially improving welfare.
