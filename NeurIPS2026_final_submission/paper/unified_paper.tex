...
We formalize this question through the class of \textbf{Tipping-Point Social Dilemmas} (TPSDs)---environments where non-linear resource dynamics create irreversible collapse thresholds---and identify a systematic failure mode we term the \textbf{Nash Trap}: individually rational policies converge to locally stable equilibria that guarantee collective extinction.
We make four contributions:
(C1)~\textbf{Nash Trap Analysis}: We prove that the Nash Trap arises from a structural mismatch between individual payoff gradients and collective survival requirements, and demonstrate that this failure is \emph{consistent across five algorithm families} (REINFORCE, PPO, IQL, QMIX, LOLA; seven implementations)---with a unified fair comparison protocol ($400{+}$ HP configurations, 100\% trap rate).
(C2)~\textbf{Commitment Floor as Safety Specification}: We derive that the minimum commitment floor $\phi_1^*$ for survival is determined by the environment's recovery function near its tipping point, establishing $\phi_1^* = 1.0$ as a \emph{stability-derived design specification} for severe TPSDs. A Lagrangian dual CPO formulation independently converges to the same floor (Appendix~\ref{app:cpo_lagrangian}).
(C3)~\textbf{Adaptive Commitment Learning (ACL)}: We introduce a two-phase algorithm that learns a \emph{state-dependent} floor $\phi_1(R_t; \omega)$, simultaneously guaranteeing survival and maximizing welfare. In environments where partial commitment is viable, ACL improves welfare by up to $241\%$ over fixed $\phi_1{=}1.0$ while maintaining $100\%$ survival (Appendix~\ref{app:acl}).
(C4)~\textbf{Generalization and External Validation}: We validate the Nash Trap and commitment floor across five environment classes---including the Coin Game~\citep{lerer2017maintaining}, a benchmark not designed by the authors---adversarial conditions (30\% Byzantine), scale ($N{=}100$), and 36 shock regimes.
Together, these results establish a \textbf{Moral Commitment Spectrum}: the degree of commitment required for collective survival is a monotonic function of environmental severity, and Adaptive Commitment Learning can discover the optimal floor automatically.
\end{abstract}
...
\begin{enumerate}
    \item[(C1)] \textbf{Nash Trap Analysis}: We define the class of Tipping-Point Social Dilemmas (Definition~\ref{def:tpsd}), prove a drift condition on resource dynamics (Lemma~\ref{lem:drift}), derive finite-horizon survival bounds (Lemma~\ref{lem:survival}), and establish the Nash Trap as a locally stable fixed point (Proposition~\ref{prop:nash_trap}) where individually rational cooperation is insufficient for collective survival.
    We demonstrate that this failure is \emph{consistent across five algorithm families} (REINFORCE, PPO, IQL, QMIX~\citep{rashid2018qmix}, and LOLA~\citep{foerster2018learning}; seven implementations including three REINFORCE variants)---all converging to $\lambda \approx 0.5$ ($\leq$72\% survival) under a unified fair comparison protocol with $400{+}$ HP configurations (Section~\ref{sec:nash_trap}). A canonical QMIX re-implementation following EPyMARL~\citep{papoudakis2021benchmarking} conventions confirms these results are implementation-invariant (Appendix~\ref{app:canonical_qmix}).
    \item[(C2)] \textbf{Commitment Floor as Safety Specification}: The optimal commitment floor $\phi_1^* = 1.0$ is derived from the environment's recovery function near its tipping point (Theorem~\ref{thm:critical}) and independently confirmed via Lagrangian dual CPO convergence (Appendix~\ref{app:cpo_lagrangian}) and systematic grid search across 110 environmental conditions (Section~\ref{sec:unconditional}). Phase diagrams visualize the Moral Commitment Spectrum.
    ...
    \item[(C4)] \textbf{Generalization and External Validation}: We validate the Nash Trap and commitment floor across five environment classes---PGG, CPR (Appendix~\ref{app:cpr}), spatial grids (Appendix~\ref{app:spatial_dilemma}), Cleanup-style commons (Appendix~\ref{app:cleanup}), and the Coin Game~\citep{lerer2017maintaining} (Appendix~\ref{app:coin_game})---adversarial conditions (30\% Byzantine), scale ($N{=}100$, Table~\ref{tab:scale}), and 36 shock regimes. A parameter sweep across 60 configurations confirms $\hat{\lambda} < 0.95$ universally (Appendix~\ref{app:nash_bound}).
\end{enumerate}

Together, these results establish a \textbf{Moral Commitment Spectrum}: moderate environments permit flexible morality; catastrophic environments with irreversible tipping points require unconditional commitment---and ACL can discover the appropriate commitment level automatically.
...
All five algorithm families fall substantially short of the oracle ($\lambda = 1.0$, 100\% survival). While we do not claim this constitutes an impossibility result for all possible algorithms, the consistency across diverse families---including value decomposition and opponent-shaping---suggests a robust pattern in this environment class.
...
\begin{itemize}
    ...
    \item \textbf{Baseline Scope}: Our comparison includes \textbf{five algorithm families} (REINFORCE, PPO, IQL, QMIX, LOLA) spanning seven implementations, including a canonical QMIX re-implementation following EPyMARL conventions (Appendix~\ref{app:canonical_qmix}). All fall into the Nash Trap, with survival ranging from 23\% to 72\%. A unified fair comparison protocol ($5{\times}5{\times}4{=}100$ HP configurations per family) confirms 100\% trap rate across all tested configurations.
...
We establish the \textbf{Moral Commitment Spectrum} through four contributions:
(C1)~The \emph{Nash Trap}---a locally stable equilibrium where individually rational policies guarantee collective extinction---arises consistently across five algorithm families ($\lambda{\approx}0.5$, $\leq$72\% survival, 100\% trap rate over $400{+}$ HP configurations), confirmed by canonical QMIX cross-validation.
(C2)~The commitment floor $\phi_1^* = 1.0$ is not an arbitrary heuristic but a \emph{stability-derived design specification}: it is mandated by the tipping-point recovery function (Theorem~\ref{thm:critical}) and independently confirmed via Lagrangian dual convergence across all tested risk tolerances.
(C3)~\emph{Adaptive Commitment Learning} (ACL) resolves the survival--welfare tradeoff by learning a state-dependent floor $\phi_1(R_t)$: in severe TPSDs, ACL recovers $\phi_1{\approx}1.0$; in moderate environments, it discovers partial floors that improve welfare by up to $241\%$ while maintaining survival.
(C4)~Both the Nash Trap and commitment floor generalize across five environment classes---including the Coin Game, a benchmark not designed by the authors---adversarial conditions (30\% Byzantine), scale ($N{=}100$), 36 shock regimes, and 60 parameter configurations.
...
\begin{enumerate}
\item \textbf{Claims}: All four contributions (C1--C4: Nash Trap Analysis, Commitment Floor as Safety Specification, Adaptive Commitment Learning, and Generalization/External Validation including the Moral Commitment Spectrum) are empirically and theoretically supported in the corresponding sections. We do not claim generality beyond the evaluated settings. [Yes]
...
\item \textbf{Experiments}:
  \begin{enumerate}
    ...
    \item Seeds: 20 for core experiments (REINFORCE, IPPO/MAPPO, IQL, QMIX, LOLA, $\phi_1$ sweep, and $N{=}100$ scale test); 10 for phase diagram and HP sweep; 5 for DNN ablation and sensitivity analysis. Seed counts are reported per experiment in the corresponding sections and appendices. [Yes]
  ...
  \end{enumerate}
\item \textbf{Code and Data}: Anonymized code repository provided. All experiments are reproducible via \texttt{reproduce\_all.py} (one-click pipeline, 10 experiments: 5 core + 5 extension). Seed counts per experiment are listed in Table~\ref{tab:seed_summary}. [Yes]
...
To validate the Nash Trap on a non-author-designed benchmark, we adapt the Coin Game~\citep{lerer2017maintaining} with shared resource dynamics: agents on a $5{\times}5$ grid collect colored coins, where picking one's own coin drains a shared resource $R_t$ by $0.04$, while picking another agent's coin (selfish extraction) drains $0.10$. Below $R_{\text{crit}}{=}0.20$, coin spawning ceases (tipping point); at $R_t{\leq}0$, the game ends.

\begin{table}[h]
\centering
\caption{Tipping-Point Coin Game (20 seeds, $T{=}100$). Even a mild tipping point creates a survival gap between selfish and committed agents.}
\label{tab:coin_game}
\begin{tabular}{lcc}
\toprule
\textbf{Policy} & \textbf{Welfare} & \textbf{Survival \%} \\
\midrule
Selfish (greedy) & $0.46$ & $94.8$ \\
Committed $\phi_1{=}0.7$ & $9.94$ & $99.9$ \\
Committed $\phi_1{=}1.0$ & $10.74$ & $99.8$ \\
ACL $\phi_1(R)$ & $8.85$ & $99.5$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding}: The Coin Game---a standard MARL cooperation benchmark not designed by the authors---exhibits the same qualitative pattern as our PGG environments: selfish agents achieve lower survival ($94.8\%$) than committed agents ($99.8\%$), and committed policies achieve dramatically higher welfare ($10.74$ vs.\ $0.46$). In this environment, the constant floor $\phi_1{=}1.0$ already achieves near-optimal welfare, so ACL offers no additional welfare benefit.
