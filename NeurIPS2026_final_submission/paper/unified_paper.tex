...
Our results decompose into three layers: (i)~\emph{tipping-point dynamics as the primary driver}---the Nash Trap persists even when $M/N \geq 1$ across all 24 tested $(N, M/N)$ conditions (Appendix~\ref{app:phase_transition}); (ii)~\emph{computational intractability} (Theorem~\ref{thm:impossibility})---escape requires $\Omega(e^{cN})$ gradient evaluations; (iii)~\emph{commitment floor as bypass}---MACCL learns the minimum sufficient floor for each environment, recovering $\phi_1 \approx 1.0$ in severe TPSDs while improving welfare by 203\% in milder environments (Appendix~\ref{app:maccl}).
