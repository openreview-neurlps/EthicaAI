...
To confirm that the Nash Trap is not an artifact of the PGG formulation, we replicate key experiments in a \textbf{Common Pool Resource (CPR)} environment (Appendix~\ref{app:cpr_detail}), which features distinct dynamics: agents extract from a shared resource with nonlinear depletion.
Under selfish RL, CPR agents converge to $\bar\lambda = 0.50$ with survival rate $9.3\%$---closely matching PGG results ($\bar\lambda \approx 0.48$, survival $\leq 15\%$).
Even with a fixed cooperative strategy ($\lambda = 0.5$), CPR survival remains at $8.5\%$, confirming that the trap is structural rather than algorithm-specific.
This cross-environment consistency supports the generality of the Nash Trap phenomenon beyond a single game formulation.
...
\textbf{Environment scope}: primary results use PGG with scalar $R_t$; CPR, Coin Game, spatial dilemma, and multi-dimensional PGG ($K{=}3$) confirm the pattern (Appendices~\ref{app:cpr_detail}--\ref{app:multi_resource}).
