...
We prove that escaping the Nash Trap requires $\Omega(e^{cN})$ gradient evaluations (Theorem~\ref{thm:impossibility}, Appendix~\ref{app:impossibility_theorem}), and that the trap persists across all tested $M/N$ ratios including $M/N = 1.0$ (Appendix~\ref{app:mn_sweep_detail}).
...
We prove that escape requires $\Omega(e^{cN})$ gradient evaluations (Theorem~\ref{thm:impossibility}) and introduce \emph{commitment floors} as a structural bypass that transforms the exponentially hard problem into a tractable constrained optimization.
