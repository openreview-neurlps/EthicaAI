...
(iii) We validate generalization on a \textbf{Grid-based Spatial Social Dilemma}, confirming that the Nash Trap phenomenon transfers to visually extended, non-linear environments where agents must navigate and harvest.
...
\begin{enumerate}
    ...
    \item[(C4)] \textbf{Generalization Beyond PGG}: We validate on a Grid-based Spatial Social Dilemma, confirming that the Nash Trap transfers to standard non-linear spatial environments (Appendix~\ref{app:spatial_dilemma}). A discrete preference network (Table~\ref{tab:preference_network}) implementing Sen's meta-ranking as ``ranking over rankings'' learns Crisis$\to$Egalitarian convergence (Appendix~\ref{app:preference_network}). Value decomposition (QMIX) and opponent-shaping (LOLA) baselines are detailed in Appendix~\ref{app:qmix_lola}.
