...
Confirmed for $N \in \{5,\ldots,100\}$. Part~(i) is proved formally; Part~(ii) is supported by rigorous empirical evidence (a full measure-theoretic proof is left to future work). The resulting Price of Anarchy diverges as $\Omega(e^{cN})$---unlike standard social dilemmas where $\mathrm{PoA} = O(1)$ (Theorem~\ref{thm:poa}; Appendix~\ref{app:poa}). Lagrangian relaxation baselines, CTDE with global critics, and neural network policies all fall into the same trap (Appendices~\ref{app:comm_ctde_neural},~\ref{app:cpr_detail}).
...
\textbf{Algorithm scope}: five algorithm families (incl.\ CleanRL IPPO/MAPPO, PyTorch 4-architecture cross-validation) all fall into the Nash Trap with 100\% trap rate.
...
\textbf{Ethics}: $\lambda$ operationalizes cooperation, not full moral reasoning~\citep{rawls1971,sen1977rational}; the commitment function $g(\theta, R)$ encodes value judgments requiring interdisciplinary deliberation.

\vspace{-2pt}
\section{Conclusion}\label{sec:broader_impact}

We identify the \textbf{Nash Trap}---a locally stable, collectively catastrophic equilibrium in TPSDs---that persists across five algorithm families, four policy architectures, $400{+}$ hyperparameter configurations, and four environment classes.
