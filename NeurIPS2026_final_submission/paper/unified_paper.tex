...
This finding is robust across five algorithm families (REINFORCE, PPO, IQL, QMIX~\citep{rashid2018qmix}, LOLA~\citep{foerster2018learning}), nine implementations, $400{+}$ hyperparameter configurations (Section~\ref{sec:fair_comparison}), and 12 $M/N$ conditions from $M/N = 0.05$ to $M/N = 1.0$ (Section~\ref{sec:mn_sweep}, Table~\ref{tab:mn_sweep}), with 100\% trap rate and 0\% escape rate across 20 seeds per condition.
...
The minimum sufficient floor $\phi_1^*$ is derived from the environment's recovery dynamics (Theorem~\ref{thm:floor}).
...
This is consistent with Theorem~\ref{thm:floor}'s derived commitment floor: the minimum $\phi_1$ that places agents outside the trap basin.
...
with our finding in \S\ref{sec:results} that a fixed commitment floor is
