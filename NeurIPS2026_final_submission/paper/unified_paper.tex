...
CPO~\citep{achiam2017constrained}, MACPO/MAPPO-Lagrangian~\citep{gu2023safe}, and CVaR objectives~\citep{garcia2015comprehensive} constrain reward optimization against \emph{exogenous} costs. Our commitment floor addresses an \emph{endogenous} failure (the Nash Trap) derived from tipping-point structure---a Lagrangian dual independently recovers $\phi_1^*{=}1.0$ (Appendix~\ref{app:cpo_lagrangian}). \citet{tennant2023modeling} embed moral theories into MARL rewards; we extend this by identifying \emph{when} each commitment level suffices for survival via the Moral Commitment Spectrum.
