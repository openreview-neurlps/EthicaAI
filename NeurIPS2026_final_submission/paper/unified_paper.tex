...
\noindent\textit{Statistical protocol}: All comparisons use seed-level aggregation: each seed's last $N_{\text{eval}}$ episodes are averaged into one observation. P-values are computed via Mann--Whitney~U with Holm--Bonferroni correction for multiple comparisons. Effect sizes are reported as Cliff's $\delta$ (see \texttt{compute_statistics.py} in the code supplement for exact values). Wilson 95\% CIs are reported throughout.
