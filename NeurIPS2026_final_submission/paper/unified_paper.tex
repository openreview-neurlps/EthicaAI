...
\begin{enumerate}
    ...
    \item[(C4)] \textbf{Generalization Beyond PGG}: We validate on DeepMind's Melting Pot benchmark (commons\_harvest, clean\_up), confirming that the Nash Trap transfers to standard high-dimensional spatial environments (Appendix~\ref{app:melting_pot}). A discrete preference network (Table~\ref{tab:preference_network}) implementing Sen's meta-ranking as ``ranking over rankings'' learns Crisis$\to$Egalitarian convergence (Appendix~\ref{app:preference_network}). Value decomposition (QMIX) and opponent-shaping (LOLA) baselines are detailed in Appendix~\ref{app:qmix_lola}.
...
A multi-axis comparison across cooperation, efficiency, fairness, scalability, and robustness dimensions (Appendix~\ref{app:multi_axis}) complements the actual Melting Pot benchmark results (Appendix~\ref{app:melting_pot}).
