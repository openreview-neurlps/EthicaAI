...
(C1)~\textbf{Nash Trap Analysis}: We prove that the Nash Trap arises from a structural mismatch between individual payoff gradients and collective survival requirements, and demonstrate that this failure is \emph{consistent across seven MARL paradigms}---REINFORCE (3 architectures), PPO (IPPO/MAPPO), IQL, QMIX, and LOLA---with a unified fair comparison protocol ($400{+}$ HP configurations, 100\% trap rate).
(C2)~\textbf{Commitment Floor as Safety Specification}: We derive that the minimum commitment floor $\phi_1^*$ for survival is determined by the environment's recovery function near its tipping point, establishing $\phi_1^* = 1.0$ as a \emph{stability-derived design specification} for severe TPSDs. A constrained policy optimization formulation independently converges to the same floor, confirming it is not an arbitrary heuristic.
(C3)~\textbf{Generalization and Robustness}: We validate the Nash Trap and commitment floor across multiple environment classes (common-pool resources, spatial grid dilemmas, Cleanup-style commons), 36 shock parameter regimes, adversarial conditions (30\% Byzantine agents), and at scale ($N{=}100$). Commitment floors simultaneously improve fairness (Gini: $0.064{\to}0.000$, worst-case return: $+87\%$).
Together, these results establish a \textbf{Moral Commitment Spectrum}: the degree of architectural commitment required for collective survival is a monotonic function of environmental severity.
\end{abstract}
...
We address these questions through a systematic investigation spanning environments of increasing severity, yielding three contributions:
\begin{enumerate}
    ...
    We demonstrate that this failure is \emph{consistent across seven MARL paradigms}: REINFORCE (3 architectures), PPO (IPPO/MAPPO), IQL, QMIX~\citep{rashid2018qmix}, and LOLA~\citep{foerster2018learning}---all converging to $\lambda \approx 0.5$ ($\leq$72\% survival) under a unified fair comparison protocol with $400{+}$ HP configurations (Section~\ref{sec:nash_trap}).
    \item[(C2)] \textbf{Commitment Floor as Safety Specification}: The optimal commitment floor $\phi_1^* = 1.0$ is derived from the environment's recovery function near its tipping point (Theorem~\ref{thm:critical}) and independently confirmed via constrained policy optimization (Lagrangian dual) convergence and systematic grid search across 110 environmental conditions (Section~\ref{sec:unconditional}). Phase diagrams visualize the Moral Commitment Spectrum: the required $\phi_1$ is a monotonic function of environmental severity.
    \item[(C3)] \textbf{Generalization and Robustness}: We validate the Nash Trap and commitment floor across multiple environment classes---common-pool resources (Appendix~\ref{app:cpr}), spatial grid dilemmas (Appendix~\ref{app:spatial_dilemma}), and Cleanup-style commons---across 36 shock parameter regimes (Appendix~\ref{app:shock_robustness}), adversarial conditions (30\% Byzantine), and at scale ($N{=}100$, Table~\ref{tab:scale}). Commitment floors simultaneously improve fairness (Gini: $0.064{\to}0.000$, Appendix~\ref{app:fairness}). Additional analyses include discrete preference networks (Appendix~\ref{app:preference_network}), crisis-only vs.\ global intervention (Appendix~\ref{app:crisis_vs_global}), and value decomposition / opponent-shaping baselines (Appendix~\ref{app:qmix_lola}).
\end{enumerate}
...
We establish the \textbf{Moral Commitment Spectrum} through three contributions:
(C1)~The \emph{Nash Trap}---a locally stable equilibrium where individually rational policies guarantee collective extinction---arises consistently across seven MARL paradigms ($\lambda{\approx}0.5$, $\leq$72\% survival, 100\% trap rate over $400{+}$ HP configurations).
(C2)~The commitment floor $\phi_1^* = 1.0$ is not an arbitrary heuristic but a \emph{stability-derived design specification}: it is mandated by the tipping-point recovery function (Theorem~\ref{thm:critical}) and independently confirmed via gradient-based constrained policy optimization (Lagrangian dual convergence).
(C3)~Both the Nash Trap and the commitment floor generalize across environment classes (PGG, CPR, Cleanup-style commons, spatial grids), adversarial conditions (30\% Byzantine), scale ($N{=}100$), and 36 shock regimes, while simultaneously improving fairness (Gini: $0.064{\to}0.000$).
