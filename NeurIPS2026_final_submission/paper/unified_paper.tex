...
We study \textbf{Tipping-Point Social Dilemmas} (TPSDs)---multi-agent settings where thresholded resource dynamics can trigger irreversible collapse---and identify an extinction-type failure we term the \textbf{Nash Trap}: gradient-based learners converge to locally stable under-commitment equilibria that are individually rational yet collectively catastrophic, \emph{even when cooperation is individually rational} ($M/N \geq 1$).

We make five contributions:
\textbf{(C1)}~We show the Nash Trap is \emph{universal across algorithms}: five families (REINFORCE, PPO, IQL, QMIX, LOLA), nine implementations, and 12 $M/N$ conditions all converge to $\lambda \approx 0.5$ with 100\% trap rate (20 seeds each).
\textbf{(C2)}~We prove escape requires $\Omega(e^{cN})$ gradient evaluations (Theorem~\ref{thm:impossibility}), via signal dilution, basin dominance, and exponential drift analysis---confirmed empirically for $N \in \{5,\ldots,100\}$.
\textbf{(C3)}~We derive commitment floors from recovery dynamics and show they bypass the barrier in $O(1)$ steps (Corollary~\ref{cor:bypass}).
\textbf{(C4)}~We introduce \textbf{MACCL} (Multi-Agent Constrained Commitment Learning), a primal-dual algorithm that learns state-dependent floors with convergence guarantees (Theorem~\ref{thm:maccl_convergence}), improving welfare by 203\% over fixed floors while maintaining 100\% survival.
\textbf{(C5)}~We validate across three environment classes (PGG, CPR, Cleanup), adversarial conditions (30\% Byzantine), and scale ($N{=}100$).

Together, these results establish the Nash Trap as a \textbf{fundamental computational barrier}---not an artifact of limited capacity---and the commitment floor as a principled, polynomial-time bypass.
\end{abstract}
...
These are not failures of intent but of \emph{individually rational} decision-making near\linebreak shared tipping points: once a critical resource crosses a collapse threshold, recovery may be slow or effectively irreversible.
...
We prove they do not---and show how to fix this.
...
\begin{enumerate}
    ...
    \item[(C5)] \textbf{Cross-Environment Validation}: We validate the framework across three structurally distinct environment classes---PGG (provision), CPR (appropriation), Cleanup (pollution)---under adversarial conditions (30\% Byzantine) and scale ($N{=}100$), \emph{plus} Google DeepMind's Melting Pot benchmark (Appendix~\ref{app:meltingpot}). The Nash Trap reproduces in all TPSD environments; MACCL generalizes across all three classes.
\end{enumerate}
