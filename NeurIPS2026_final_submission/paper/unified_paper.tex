...
\textbf{Algorithm scope}: we test 12 implementations across 5 algorithm families plus CleanRL IPPO/MAPPO (4,546 params) and PyTorch REINFORCE cross-validation (3 architectures, Beta distribution, Adam optimizer); all fall into the Nash Trap. CTDE with global-state critics and communication channels are tested (Appendix~\ref{app:comm_ctde_neural}): 100\% trap rate. PyTorch results ($\bar\lambda \approx 0.48\text{--}0.50$) closely match NumPy baselines ($\bar\lambda \approx 0.48$), confirming framework independence.
\textbf{Environment scope}: primary results use PGG with scalar $R_t$; three additional environment classes---CPR (extraction), Harvest (resource depletion), and Cleanup (maintenance allocation)---confirm the pattern across structurally distinct dilemmas (Table~\ref{tab:cross_env}). Multi-dimensional PGG ($K{=}3$) is tested in Appendix~\ref{app:multi_resource}.
...
We identify the \textbf{Nash Trap}---a locally stable, collectively catastrophic equilibrium in Tipping-Point Social Dilemmas---that persists across 14 algorithm variants (including CleanRL IPPO/MAPPO with 4,546-parameter MLPs and PyTorch REINFORCE with Beta distributions), $400{+}$ hyperparameter configurations, and multiple environment classes.
We establish that escape requires $\Omega(e^{cN})$ gradient evaluations (Proposition~\ref{thm:impossibility}: signal dilution proved formally; basin dominance and escape time supported by comprehensive empirical evidence) and introduce \emph{commitment floors} as a structural bypass that transforms the exponentially hard problem into a tractable constrained optimization.
...
The broader implication is a design principle: when tipping-point dynamics make selfish learning structurally misaligned with survival, commitment must be imposed as a stability-derived safety specification rather than learned from misaligned gradients.
