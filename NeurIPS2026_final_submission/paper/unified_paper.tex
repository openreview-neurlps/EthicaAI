...
\paragraph{Comparison with Safe RL Baselines.}
A natural question is whether standard constrained policy optimization methods---which enforce \emph{exogenous} safety constraints---can escape the Nash Trap without a structural commitment floor.
We compare against CPO-style~\citep{achiam2017constrained} and MACPO-style~\citep{gu2023safe} baselines, both configured with the same survival constraint ($P_{\mathrm{surv}} \geq 95\%$), on our severe TPSD ($N{=}20$, Byz$=$30\%, 20 seeds $\times$ 300 episodes).

\begin{table}[h]
\centering
\small
\caption{Safe RL comparison: exogenous constraint (CPO/MACPO) vs.\@ endogenous floor (MACCL). Both CPO methods converge to the Nash Trap ($\lambda \approx 0.5$); only the structural floor escapes.}
\label{tab:cpo_comparison}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Method} & \textbf{Type} & $\boldsymbol{\bar{\lambda}}$ / \textbf{Floor} & \textbf{Survival} & \textbf{Welfare} \\
\midrule
CPO (exog.) & Lagrangian RL & $0.526$ & 56.3\% & 21.26 \\
MACPO (exog.) & Multi-agent Lagrangian & $0.484$ & 50.8\% & 21.16 \\
\midrule
...
\noindent Both CPO and MACPO converge to $\bar{\lambda} \approx 0.5$---squarely within the Nash Trap---despite the survival constraint.
...
In contrast, MACCL's structural floor directly excludes the trap basin from the feasible action space, bypassing the local gradient entirely.
