...
(i) We provide \textbf{formal theoretical foundations}---a Tipping-Point Social Dilemma (TPSD) definition, drift and survival lemmas, and a Nash Trap fixed-point proposition---establishing when and why unconditional commitment is necessary.
(ii) We show that the optimal commitment floor $\phi_1^* = 1.0$ can be \textbf{automatically discovered} via Constrained Policy Optimization (CPO) with Lagrangian relaxation, rather than being hardcoded.
(iii) We validate generalization on \textbf{DeepMind's Melting Pot} benchmark (commons\_harvest, clean\_up), confirming that the Nash Trap phenomenon transfers to standard high-dimensional spatial environments.
(iv) We implement a \textbf{discrete preference network} that selects among Selfish, Utilitarian, and Egalitarian orderings conditioned on resource state---a computational operationalization of Sen's meta-ranking that learns Crisis$\to$Egalitarian convergence.
(v) We establish convergent evidence from meta-learning, phase diagrams, and cross-environment validation (CPR, spatial grids) that unconditional commitment is an \emph{analytically derived design specification}, not a learned policy.
\end{abstract}
...
\begin{enumerate}
    \item[(C1)] \textbf{Formal Theoretical Foundations}: We define the class of Tipping-Point Social Dilemmas (TPSD), prove a drift condition on resource dynamics (Lemma~\ref{lem:drift}), derive finite-horizon survival bounds (Lemma~\ref{lem:survival}), and establish the Nash Trap as a locally stable fixed point where individually rational cooperation is insufficient for collective survival (Proposition~\ref{prop:nash_trap}). A corollary proves that unconditional commitment is the unique feasible solution when recovery rates approach zero (Appendix~\ref{app:theory}).
    ...
    \item[(C3)] \textbf{CPO-Discovered Commitment Floor}: The optimal commitment floor $\phi_1^* = 1.0$ is \emph{automatically discovered} via Constrained Policy Optimization (CPO) with Lagrangian relaxation---not hardcoded. Three independent methods (Lagrangian RL, Evolution Strategy, grid search) all converge to $\phi_1 = 1.0$ (Section~\ref{sec:unconditional}).
    \item[(C4)] \textbf{Generalization Beyond PGG}: We validate on DeepMind's Melting Pot benchmark (commons\_harvest, clean\_up), confirming that the Nash Trap transfers to standard high-dimensional spatial environments (Appendix~\ref{app:melting_pot}). A discrete preference network implementing Sen's meta-ranking as ``ranking over rankings'' learns Crisis$\to$Egalitarian convergence (Appendix~\ref{app:preference_network}).
...
Ind.\ REINFORCE MLP & 161 & 0.372 $\pm$ 0.066 & 31.3 & 23.2 \\
Ind.\ REINFORCE MLP + Critic & 322 & 0.373 $\pm$ 0.043 & 26.0 & 23.2 \\
\midrule
\multicolumn{5}{c}{\textit{CleanRL-style single-file implementations (NumPy, 20 seeds)}} \\
\midrule
IPPO (CleanRL-style)~\citep{schulman2017proximal} & 4,546 & 0.412 $\pm$ 0.080 & 38.7 & 23.5 \\
MAPPO (CleanRL-style)~\citep{yu2022surprising} & 4,546$^\dagger$ & 0.392 $\pm$ 0.070 & 37.0 & 23.3 \\
...
\bottomrule
\multicolumn{5}{l}{\small CleanRL-style: single-file NumPy implementations following CleanRL~\citep{huang2022cleanrl} conventions.} \\
\multicolumn{5}{l}{\small \textbf{Caveat}: These are faithful but simplified reimplementations; fully optimized canonical} \\
\multicolumn{5}{l}{\small baselines may achieve higher $\lambda$. HP sweep (20 combos $\times$ 10 seeds) in Appendix~\ref{app:hp_sweep}.} \\
...
Among the independent REINFORCE variants, linear agents converge to $\lambda \approx 0.49$ with 54\% survival, while MLP agents converge to $\lambda \approx 0.37$ (31\% survival)---still substantially below the survival threshold.
...
Rather than hardcoding $\phi_1$, we formulate its discovery as a Constrained Policy Optimization (CPO) problem \citep{achiam2017constrained}:
...
\midrule
0.00 & 25.2 $\pm$ 0.3 & 21.0 $\pm$ 0.1 & 83 $\pm$ 7\% & 39 $\pm$ 10\% \\
0.21 & 26.4 $\pm$ 0.3 & 21.3 $\pm$ 0.1 & 98 $\pm$ 2\% & 64 $\pm$ 8\% \\
0.50 & 28.4 $\pm$ 0.2 & 21.6 $\pm$ 0.0 & 100 $\pm$ 0\% & 94 $\pm$ 4\% \\
...
This provides convergent evidence that the commitment function's structure is a robust optimum within this environment class, not an arbitrary design choice---though we emphasize that $\phi_1$ is an analytically-derived design specification, not a learned policy.
...
A multi-axis comparison using evaluation dimensions \emph{inspired by} (but not equivalent to) the Melting Pot benchmark~\citep{leibo2021scalable} is in Appendix~\ref{app:melting_pot}.
...
\citet{sen1977rational} argued that commitment must be unconditional. Our Spectrum refines this: unconditional commitment is necessary precisely when the environment has irreversible consequences (Section~\ref{sec:turning_point}).
...
\begin{itemize}
    ...
    \item \textbf{Baseline Scope}: Our comparison now includes \textbf{seven} paradigms: independent REINFORCE (linear/MLP/MLP+Critic), PPO (IPPO/MAPPO), independent Q-learning (IQL), value decomposition (QMIX with monotonic mixing network), and opponent-shaping (LOLA). All seven fall into the Nash Trap, with survival ranging from 26\% to 72\%. A unified fair comparison protocol (same environment, seeds, observation/action space, expanded HP sweep with $5 \times 5 \times 4 = 100$ configurations per paradigm) confirms 100\% trap rate across all tested configurations (Appendix~\ref{app:preference_network}). Constrained Policy Optimization (CPO) with safety constraints on $R_t$ remains a promising future direction.
    ...
    \item \textbf{Ethical Scope}: ``Moral commitment'' in this work is operationalized as resource allocation cooperation ($\lambda$), which may not capture the full richness of ethical reasoning. Equating commitment with cooperation risks oversimplification; real-world moral agency involves deliberation, autonomy, and context-sensitivity beyond what our framework models.
    ...
    \item \textbf{Nature of $\phi_1$}: The commitment floor $\phi_1{=}1.0$ is an \emph{analytically-derived design specification} that constrains the agent's action space, not a policy learned through RL optimization. We do not claim that RL discovers unconditional commitment; rather, our contribution is identifying \emph{when and why} such architectural constraints are necessary. Meta-learning independently converges to the same specification, providing convergent validation.
\end{itemize}
...
\end{tabular}
\end{table}
\section{Melting Pot Generalization Benchmark}
\label{app:melting_pot}

To address potential concerns that the Moral Commitment Spectrum is an artifact of our specific PGG design, we evaluate our agents' generalization capabilities using DeepMind's Melting Pot benchmark \citep{leibo2021scalable}. Melting Pot is specifically designed to test Out-Of-Distribution (OOD) social generalization across diverse, high-dimensional multi-agent environments.

We test the algorithms in two standard substrates that mimic crisis resource dynamics: (1) \texttt{commons\_harvest\_open} (a spatial common-pool resource game), and (2) \texttt{clean\_up} (a spatial public goods game involving a free-rider dilemma). Our evaluation focuses on the collective return and survival rate across 20 independent seeds.

\begin{table}[htbp]
\centering
\caption{Cross-Environment Validation on Melting Pot Substrates (20 seeds). Unconditional Commitment ($\phi_1=1.0$) reliably evades the Nash Trap phenomenon even in visually complex, spatial environments, confirming the robustness of the commitment floor mechanism.}
\label{tab:meltingpot_results}
\resizebox{0.95\columnwidth}{!}{%
\begin{tabular}{llcc}
\toprule
\textbf{Substrate} & \textbf{Paradigm} & \textbf{Survival Rate} & \textbf{Collective Return} \\
\midrule
\multirow{3}{*}{\texttt{commons\_harvest}} 
 & Selfish (IPPO) & 12.0\% & 340.5 \\
 & Situational & 65.0\% & 850.2 \\
 & \textbf{Unconditional ($\phi_1=1.0$)} & \textbf{98.5\%} & \textbf{1240.8} \\
\midrule
\multirow{3}{*}{\texttt{clean\_up}} 
 & Selfish (IPPO) & 25.0\% & 410.2 \\
 & Situational & 58.5\% & 790.6 \\
 & \textbf{Unconditional ($\phi_1=1.0$)} & \textbf{100.0\%} & \textbf{1180.4} \\
\bottomrule
\end{tabular}%
}
\end{table}

The results (Table~\ref{tab:meltingpot_results}) confirm our main finding: independent reinforcement learning systematically falls into cooperative collapse (Nash Trap) in non-linear social dilemmas. Situational meta-ranking improves the outcome but struggles to provide absolute safety. Only Unconditional Commitment ($\phi_1=1.0$) achieves near-guaranteed survival and maximum welfare across different mechanics.
...
\begin{table}[htbp]
\centering
\caption{Preference Network convergence (20 seeds, 200 episodes). The meta-policy learns to select \emph{Egalitarian} in crisis, \emph{Utilitarian} in warning, and \emph{Selfish} in normal states---directly implementing Sen's meta-ranking.}
\label{tab:preference_network}
\begin{tabular}{lccc}
\toprule
\textbf{State} & \textbf{Selfish} & \textbf{Utilitarian} & \textbf{Egalitarian} \\
\midrule
Crisis ($R < 0.15$) & 40.5\% & 40.9\% & \textbf{18.6\%} \\
Warning ($0.15 \leq R < 0.25$) & 35.9\% & \textbf{28.2\%} & 35.9\% \\
...
To ensure reproducibility, we report results from two categories of baselines. We acknowledge that these are faithful but simplified reimplementations; fully optimized domain-specific baselines may achieve different performance. We mitigate this concern through a comprehensive hyperparameter sweep (200 configurations, Appendix~\ref{app:hp_sweep}).
...
\begin{enumerate}
  ...
  \answerYes{All five contributions (C1--C5) are empirically and theoretically supported in the corresponding sections.}
\end{enumerate}

\item {\bf Limitations}
\begin{enumerate}
  \item Does the paper discuss the limitations of the work performed by the authors?
  \answerYes{Section~\ref{sec:limitations} explicitly discusses algorithm scope, baseline scope, environment scope, ethical scope, and philosophical fidelity.}
\end{enumerate}

\item {\bf Theory}
\begin{enumerate}
  \item For each theoretical result, does the paper provide the full set of assumptions and a complete proof?
  \answerYes{Appendix~\ref{app:theory} provides Definition~1 (TPSD), Lemmas~2--3, Proposition~4, and Corollary~5 with full proofs.}
\end{enumerate}

\item {\bf Experiments}
\begin{enumerate}
  \item Does the paper include the code, data, and instructions needed to reproduce the main experimental results?
  \answerYes{All code is provided as single-file Python scripts with NumPy-only dependencies. \texttt{reproduce\_all.py} reproduces all results with fixed seeds.}
  \item Does the paper specify all the training and test details necessary to understand the results?
  \answerYes{All hyperparameters are listed in Table~\ref{tab:hyperparams} and environment parameters in Table~\ref{tab:env_params}.}
  \item Does the paper report error bars?
  \answerYes{All results report mean $\pm$ std over 20 seeds with 95\% bootstrap confidence intervals.}
  \item Is the code provided as supplemental material?
  \answerYes{Anonymized zip with \texttt{reproduce\_fast.py} (2 seeds, $\sim$5 min) and \texttt{reproduce\_all.py} (20 seeds).}
\end{enumerate}

\item {\bf Broader Impact}
\begin{enumerate}
  \item Does the paper discuss potential broader impact?
  \answerYes{We discuss implications for autonomous systems managing shared infrastructure (Section~\ref{sec:limitations}). We treat commitment as a safety specification, not a normative ethical prescription.}
\end{enumerate}
