...
\textbf{Result}: The Nash Trap is a robust structural barrier across \textbf{seven evaluated paradigms} (Table~\ref{tab:emergence}).
...
All seven paradigms fall substantially short of the oracle ($\lambda = 1.0$, 100\% survival), confirming that the Nash Trap is a fundamental property of the non-linear PGG, not an algorithmic deficiency.
...
\paragraph{Policy Implications: From PGG to Climate Governance.}
Our framework admits a direct mapping to real-world common-pool resource problems.
The tipping point $R_{\text{crit}} = 0.15$ corresponds structurally to planetary boundaries (e.g., the 1.5\textdegree{}C warming threshold~\citep{ipcc2018}): once crossed, recovery dynamics slow catastrophically ($f(R) \to 0.01$), mirroring the irreversibility of permafrost methane release or ice-sheet disintegration.
The commitment floor $\phi_1$ maps to \textbf{mandatory minimum cooperation levels}---analogous to the Paris Agreement's Nationally Determined Contributions (NDCs), which set binding emission reduction floors regardless of short-term economic incentives.
A full ablation is in Appendix~\ref{app:ablation}. Theorem~\ref{thm:critical} provides the engineering specification: in our model, $\phi_1^* \geq 1.64$ (truncated to 1.0), meaning \textbf{partial commitment is provably insufficient} when the recovery rate at the tipping point is small.
For AI governance, this suggests that \textbf{high-risk autonomous systems managing shared resources} (e.g., autonomous fishing fleets, energy grid agents, carbon credit markets) should be designed with hard-coded cooperation floors rather than relying on learned cooperation, which our seven-paradigm analysis shows consistently fails.
