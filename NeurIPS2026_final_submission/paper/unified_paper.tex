...
\textbf{Discovery.}~The Nash Trap is universal across every algorithm we tested: five families (REINFORCE, PPO, IQL, QMIX, LOLA), nine implementations, 400{+} hyperparameter configurations, and 12 $M/N$ conditions all converge to $\lambda \approx 0.5$ with 100\% trap rate (20 seeds each). Neither value decomposition, opponent shaping, nor reward shaping escapes it.
\textbf{Explanation.}~We show escape requires $\Omega(e^{cN})$ gradient evaluations via three mechanisms: signal dilution ($O(\epsilon/N)$ survival gradient vs.\ $O(1)$ reward noise), basin dominance (trap basin has measure $\geq 1 - e^{-cN}$), and exponential drift---confirmed for $N \in \{5,\ldots,100\}$.
\textbf{Prescription.}~We derive minimum commitment floors from recovery dynamics and show they bypass the trap in $O(1)$ steps by excluding the trap basin from the feasible action space. MACCL, a primal-dual extension, learns state-dependent floors adaptively, improving welfare by 203\% in environments where partial commitment suffices.

These results are validated across three environment classes (PGG, CPR, Cleanup), multi-dimensional action spaces ($K{=}3$), adversarial conditions (30\% Byzantine), and scale ($N{=}100$), establishing the Nash Trap as a \textbf{systematic barrier for gradient-based learners} in tipping-point environments, and the commitment floor as a principled bypass.
\end{abstract}
...
This finding is robust across five algorithm families (REINFORCE, PPO, IQL, QMIX~\citep{rashid2018qmix}, LOLA~\citep{foerster2018learning}), nine implementations, $400{+}$ hyperparameter configurations (Section~\ref{sec:experiments}), and 12 $M/N$ conditions from $M/N = 0.05$ to $M/N = 1.0$ (Section~\ref{sec:mn_sweep}, Table~\ref{tab:mn_sweep}), with 100\% trap rate and 0\% escape rate across 20 seeds per condition.
...
We identify three mechanisms underlying the Nash Trap (Theorem~\ref{thm:impossibility}):
(i)~\emph{signal dilution}---the survival gradient is $O(\epsilon / N)$ against reward noise $O(1)$;
(ii)~\emph{basin dominance}---the trap basin has measure $\geq 1 - e^{-c_1 N}$ under symmetric initialization;
(iii)~\emph{exponential drift}---escape requires $\Omega(e^{cN})$ gradient evaluations.
These results are confirmed empirically across $N \in \{5, 10, 20, 50, 100\}$ (Section~\ref{sec:impossibility}).
We also show that Lagrangian relaxation baselines---which impose survival constraints exogenously---converge to $\bar\lambda \approx 0.5$ in the same trap (Table~\ref{tab:cpo_comparison}), demonstrating that the problem is not merely unconstrained optimization but a structural property of the gradient landscape.
Furthermore, neither cheap-talk communication, centralized critics (CTDE), nor neural network policies escape the trap (Appendix~\ref{app:comm_ctde_neural}): CleanRL-style IPPO/MAPPO with 4{,}546-parameter MLPs achieve \emph{even lower} $\bar\lambda \approx 0.40$ than simple REINFORCE ($\approx 0.50$), with 100\% trap rate across 20 seeds each.

\paragraph{3.~Prescription: Commitment Floors.}
Since the trap cannot be escaped by gradient methods, we derive a structural bypass: \emph{commitment floors} that exclude the trap basin from the feasible action space (Corollary~\ref{cor:bypass}).
The minimum sufficient floor $\phi_1^*$ is derived from the environment's recovery dynamics (Theorem~\ref{thm:critical}).
As a practical extension, MACCL (Multi-Agent Constrained Commitment Learning) learns state-dependent floors via primal-dual optimization, improving welfare by 203\% over fixed floors in environments where partial commitment suffices (Section~\ref{sec:maccl}).

These results are validated across three environment classes (PGG, CPR, Cleanup), multi-dimensional actions ($K{=}3$; Appendix~\ref{app:multi_resource}), adversarial conditions (30\% Byzantine), and scale ($N{=}100$), establishing the Nash Trap as a \textbf{systematic barrier for independent gradient-based learners} in tipping-point environments.
...
\paragraph{Distinction from Known Cooperation Failures.}
The observation that RL agents fail to cooperate in social dilemmas is well-documented~\citep{leibo2017multi, hughes2018inequity}.
Our contribution is not this observation itself, but three qualitative departures:
(1)~\emph{Irreversibility}: prior work studies matrix games and sequential dilemmas that reset each episode; our environments feature \emph{within-episode} resource collapse that is near-irreversible ($f(R_{\mathrm{crit}}) = 0.01$), making failure qualitatively different from low-reward outcomes.
(2)~\emph{Formal characterization}: prior work reports cooperation failure empirically; we characterize the Nash Trap as a \emph{locally stable fixed point} with quantified location ($\lambda \approx 0.5$), basin measure ($\geq 1 - e^{-cN}$), and escape complexity ($\Omega(e^{cN})$ gradient evaluations).
(3)~\emph{Structural bypass}: existing solutions modify \emph{rewards} (intrinsic motivation, opponent shaping); we modify the \emph{feasible action space} via commitment floors---a categorically different intervention that is provably polynomial-time (Corollary~\ref{cor:bypass}).
...
In the multi-agent setting, MACPO and MAPPO-Lagrangian~\citep{gu2023safe} extend constrained optimization to cooperative MARL, while RCPO~\citep{tessler2019reward} transforms constraints into reward penalties.
These methods assume the constraint (e.g., collision avoidance) is \emph{exogenous and differentiable}.
In contrast, our constraint---the commitment floor---is \emph{derived from the environment's tipping-point structure} and addresses an \emph{endogenous} failure mode (the Nash Trap) that no unconstrained method escapes in our tests.
We formulate the commitment problem through the constrained lens: a Lagrangian dual approach independently recovers $\phi_1^* = 1.0$ as the safety-optimal floor (Eq.~\ref{eq:cpo_formulation}), and MACCL extends this with formal convergence guarantees (Theorem~\ref{thm:maccl_convergence}).
Crucially, while CPO constrains a single agent's cost to remain below a threshold, MACCL constrains all agents' \emph{minimum contribution} via a collective floor---transforming the exponentially hard coordination problem (Theorem~\ref{thm:impossibility}) into a tractable constrained optimization.
...
\subsection{Situational Commitment in Linear Environments}
\label{sec:linear}
...
\paragraph{LOLA Implementation Caveat.}
Our $N$-agent LOLA uses a 1-step same-trajectory approximation that may underestimate LOLA's full opponent-shaping capability.
However, we note that (i)~a faithful $N$-agent LOLA requires $\mathcal{O}(N^2)$ Hessian computations per step, which is computationally intractable at $N{=}20$; (ii)~this scalability barrier is itself evidence that opponent-shaping methods are impractical for the multi-agent settings where the Nash Trap is most dangerous; and (iii)~at $N{=}2$ where faithful LOLA is feasible, the optimal strategy in our TPSD is still $\lambda{=}1.0$, which LOLA does not discover (Appendix~\ref{app:kpg}).
...
\begin{enumerate}
    \item \textbf{Tipping-point dynamics as the primary driver}: The Nash Trap persists \emph{independently of the $M/N$ ratio}---even when $M/N \geq 1$ (cooperation is individually rational in expectation), gradient learners converge to $\lambda \approx 0.5$ with 100\% trap rate across all 24 tested $(N, M/N)$ conditions (Section~\ref{sec:mn_sweep}, Figure~\ref{fig:phase_diagram_2d}). The non-linear collapse dynamics, not the payoff structure, are the root cause.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/phase_diagram_2d.png}
    \caption{Nash Trap phase diagram across agent count $N \in \{5, 10, 20, 50\}$ and $M/N$ ratio $\in \{0.05, 0.15, 0.30, 0.50, 0.75, 1.00\}$ (24 conditions, 10 seeds $\times$ 200 episodes, Byz=30\%).
    \textbf{(a)} Converged commitment $\lambda$: all conditions converge to $\lambda \approx 0.50$, confirming the Nash Trap is $M/N$-independent.
    \textbf{(b)} Survival rate: higher at $N{=}5$ (70--75\%) due to reduced free-rider impact, but no condition achieves the oracle survival of 100\%.
    Together, these results establish a 100\% trap rate across the entire $(N, M/N)$ parameter space.}
    \label{fig:phase_diagram_2d}
\end{figure}
...
\paragraph{Results.}
In severe TPSDs (non-linear PGG), MACCL recovers $\phi_1 \approx 0.83$, achieving 86.7\% survival with welfare comparable to fixed $\phi_1{=}1.0$.
...
\paragraph{Comparison with Constrained RL Baselines.}
A natural question is whether standard constrained optimization methods---which enforce \emph{exogenous} safety constraints on the policy---can escape the Nash Trap without a structural commitment floor.
We implement Lagrangian relaxation baselines in both single-agent and multi-agent variants, configured with the same survival constraint ($P_{\mathrm{surv}} \geq 95\%$), on our severe TPSD ($N{=}20$, Byz$=$30\%, 20 seeds $\times$ 300 episodes).\footnote{We use Lagrangian relaxation rather than the full trust-region CPO~\citep{achiam2017constrained} because our scalar action space ($\lambda \in [0,1]$) makes trust regions unnecessary---the Lagrangian formulation captures the essential constraint-penalized mechanism. Extending to neural-policy CPO in higher-dimensional action spaces is future work (see Phase 2, Appendix~\ref{app:multi_resource}).}

\begin{table}[h]
\centering
\small
\caption{Constrained RL comparison: Lagrangian relaxation (exogenous constraint) vs.\@ commitment floor (endogenous). Both Lagrangian methods converge to the Nash Trap ($\lambda \approx 0.5$); only the structural floor escapes.}
\label{tab:cpo_comparison}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Method} & \textbf{Type} & $\boldsymbol{\bar{\lambda}}$ / \textbf{Floor} & \textbf{Survival} & \textbf{Welfare} \\
\midrule
Lagrangian RL & Exog.\ constraint & $0.526$ & 56.3\% & 21.26 \\
Multi-agent Lagrangian & Exog.\ + shared cost & $0.484$ & 50.8\% & 21.16 \\
\midrule
MACCL (endog.) & Floor $\phi_1(R;\omega)$ & $0.829$ & \textbf{86.7\%} & \textbf{21.99} \\
Fixed $\phi_1{=}1.0$ & Unconditional & $1.000$ & 100.0\% & 22.40 \\
\bottomrule
\end{tabular}
\end{table}

\noindent Both Lagrangian methods converge to $\bar{\lambda} \approx 0.5$---squarely within the Nash Trap---despite the survival constraint.
The constraint violation signal ($P_{\mathrm{surv}} < 95\%$) increases the Lagrange multiplier, which pushes the policy toward higher $\lambda$, but this effect is overwhelmed by the myopic anti-commitment gradient (Proposition~\ref{prop:gradient_impossibility}).
This demonstrates that the Nash Trap is not merely a failure of unconstrained optimization: even when survival is explicitly constrained, the local gradient landscape prevents escape without structural intervention.
In contrast, MACCL's endogenous floor directly excludes the trap basin from the feasible action space, bypassing the local gradient entirely.
...
\begin{itemize}
    ...
    \emph{On environment complexity}: our primary PGG uses scalar $R_t$ and $\lambda_i \in [0,1]$ by design---the minimal environment isolates the Nash Trap mechanism from confounding factors such as partial observability, communication, and spatial structure. We validate that the Nash Trap persists under multi-dimensional action spaces: in a $K{=}3$ Multi-Resource PGG where each agent outputs three commitment levels ($\lambda_i^{(k)} \in [0,1]^3$, 10D observation), selfish agents converge to $\bar{\lambda} = 0.499$ with 12\% survival across 20 seeds---the same trap as in scalar PGG (Appendix~\ref{app:multi_resource}). Additionally, MACCL-Neural, an MLP-parameterized floor with 579 learned parameters, achieves 80.2\% survival in this setting, confirming that the commitment floor mechanism scales beyond hand-tuned parametrizations.
...
To directly validate the basin dominance claim in Theorem~\ref{thm:impossibility}, we vary the initial commitment level $\lambda_0 \in \{0.10, 0.30, 0.50, 0.70, 0.85, 0.90, 0.95, 1.00\}$ and measure whether agents converge to the Nash Trap or maintain cooperation ($N{=}20$, 30\% Byzantine, 20 seeds, 300 episodes).
...
\midrule
CleanRL IPPO (4,546 params/agent) & $0.409$ & 37.5\% & 100\% \\
CleanRL MAPPO (4,546 + shared critic) & $0.394$ & 37.0\% & 100\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Remarkably, CleanRL IPPO/MAPPO achieve \emph{lower} $\bar\lambda$ ($\approx 0.40$) than simple REINFORCE ($\approx 0.50$)}, suggesting that stronger optimization capacity drives agents \emph{deeper} into the trap by more effectively exploiting the myopic gradient signal.
