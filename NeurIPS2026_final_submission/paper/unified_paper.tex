...
\begin{enumerate}
    ...
    \item[(C2)] \textbf{Commitment Floor as Safety Specification}: The required commitment floor is derived from the environment's recovery function near its tipping point (Theorem~\ref{thm:critical}). In severe regimes the required floor exceeds the feasible range and saturates at $\phi_1 = 1.0$; this is independently confirmed via Lagrangian dual CPO convergence (Appendix~\ref{app:cpo_lagrangian}) and grid search across 110 conditions (Section~\ref{sec:unconditional}). Phase diagrams visualize the Moral Commitment Spectrum.
    \item[(C3)] \textbf{Adaptive Commitment Learning (ACL)}: While $\phi_1{=}1.0$ is \emph{survival-optimal}, it can be \emph{welfare-suboptimal}. We introduce ACL, a two-phase algorithm that learns a state-dependent floor $\phi_1(R_t; \omega) = \sigma(\omega_1 R_t + \omega_2 R_t^2 + \omega_3)$ via safety-constrained welfare optimization. In Cleanup-style environments, ACL improves welfare by $241\%$ over fixed $\phi_1{=}1.0$ while maintaining $100\%$ survival; in severe TPSDs, it automatically recovers $\phi_1 \approx 1.0$ (Appendix~\ref{app:acl}).
    \item[(C4)] \textbf{Cross-Environment Validation and Boundary Tests}: We validate the Nash Trap and commitment floor across five environment classes sharing incentive misalignment and thresholded collapse---PGG, CPR (Appendix~\ref{app:cpr}), spatial grids (Appendix~\ref{app:spatial_dilemma}), Cleanup-style commons (Appendix~\ref{app:cleanup}), and the Coin Game~\citep{lerer2017maintaining} (Appendix~\ref{app:coin_game})---under adversarial conditions (30\% Byzantine), scale ($N{=}100$, Table~\ref{tab:scale}), and 36 shock regimes. Environments lacking these ingredients do not show a commitment-floor advantage in our tests (Section~\ref{sec:linear}; Appendix~\ref{app:meltingpot_prelim}). A parameter sweep across 60 configurations confirms $\hat{\lambda} < 0.95$ universally (Appendix~\ref{app:nash_bound}).
\end{enumerate}
...
We formulate the commitment problem through this constrained lens: a Lagrangian dual approach independently recovers $\phi_1^* = 1.0$ as the safety-optimal floor, validating our design specification.
...
Situational policies can still achieve high \emph{average} survival when trajectories rarely enter the crisis region (e.g., Table~\ref{tab:scale}), but their crisis response is structurally misaligned with worst-case safety.
Lemma~\ref{lem:adaptive_latency} formalizes this: the exponential smoothing parameter $\alpha$ creates an effective response time $\tau_\alpha = \lceil 1/(1-\alpha) \rceil$ during which burst shocks accumulate unopposed, yielding an exponentially higher collapse probability than unconditional commitment ($\tau_0 = 1$).
...
\noindent This does not claim impossibility in general---long-horizon return gradients include survival effects that partially counteract ($\partial p_{\text{surv}}/\partial \lambda > 0$)---but it explains why the Nash Trap is \emph{robust}: the local anti-commitment signal must be overcome at every time step. Our experiments confirm that neither reward shaping (Table~\ref{tab:team_reward}), extended horizons (Table~\ref{tab:long_horizon}), nor opponent-shaping algorithms empirically overcome this barrier.
...
situational commitment achieves 6.5\% (worse than selfish, due to \emph{restraint decay}: the crisis rule reduces restraint exactly when the tipping-point recovery rate is slowest, accelerating depletion faster than pure selfishness);
