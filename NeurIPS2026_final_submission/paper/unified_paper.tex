...
QMIX$^\S$~\citep{rashid2018qmix} & 7,948$^\|$ & 0.504 $\pm$ 0.144 & 68.6 & 22.8 \\
...
\multicolumn{5}{l}{\small $^\|$Per-agent Q + mixing net. Canonical PyTorch QMIX (20 seeds) confirms results (Appendix~\ref{app:canonical_qmix}).} \\
\multicolumn{5}{l}{\small $^{\#}$LOLA: 2 seeds; LOLA's $O(N^2)$ cross-derivative computation limits scaling. All other baselines: 20 seeds.} \\
\end{tabular}}
\footnotetext{$E{=}20$. Mann-Whitney $U$ with Holm--Bonferroni correction ($K{=}5$ vs.\ oracle): Cliff's $|\delta|{=}1.0$ for all pairs (maximum separation). 95\% CIs (20 seeds): IPPO $\lambda$ [0.368, 0.422]; MAPPO [0.213, 0.236]; IQL [0.575, 0.593]; QMIX [0.360, 0.648]. Wall-clock: 18--198s/seed (CPU). Full output: \texttt{cleanrl\_baseline\_results.json}.}
\end{table}
...
Notably, \textbf{QMIX with a monotonic mixing network} ($\lambda = 0.504 \pm 0.144$, 68.6\% survival, canonical cross-validation with 20 seeds) does not outperform IQL, indicating that value decomposition does not help escape the Nash Trap.
