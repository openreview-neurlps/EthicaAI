...
where $J_{\mathrm{myopic}}$ has eigenvalues $\approx -(1-M/N)$ (stabilizing)
and $J_{\mathrm{surv}}$ has eigenvalues $O(\epsilon / N^2)$ (negligible).
Thus $J$ is negative definite, making $\hat{\lambda}$ locally asymptotically stable
(Stable Manifold Theorem; Shub, 1987).
Escape requires simultaneous perturbation of $\Omega(N)$ agents in the
cooperative direction, which occurs with probability $e^{-\Omega(N)}$
