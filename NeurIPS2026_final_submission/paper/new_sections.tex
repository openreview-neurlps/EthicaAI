...
\begin{enumerate}
  ...
  \item[\textbf{(ii)}] \textbf{Basin dominance.}
    ...
    $\mu(\mathcal{B}) \geq 1 - e^{-c_1 N}$ for some $c_1 > 0$
...
Parts~(ii) and~(iii) are supported by comprehensive empirical evidence across $N \in \{5, 10, 20, 50, 100\}$ with 20 seeds each: 100\% trap convergence from uniform initialization (basin dominance) and zero escapes in 500+ episodes (escape hardness).
A rigorous mathematical foundation for Parts~(ii)--(iii) using Freidlin--Wentzell large deviations theory and continuous-time stochastic approximation is provided in Appendix~\ref{app:impossibility_proof}.
\end{remark}
...
\textbf{Part (ii).}
At $\hat{\lambda}$, the Jacobian of the gradient dynamics decomposes as
...
\emph{Case $M/N < 1$}: $J$ is negative definite, making $\hat{\lambda}$ locally asymptotically stable
(Stable Manifold Theorem; Shub, 1987).
\emph{Case $M/N = 1$}: the myopic eigenvalue degenerates to zero; however, the sigmoid parametrization induces a stabilizing curvature:

\begin{proposition}[Stochastic Stability at $M/N{=}1$]
\label{prop:mn1_stability}
When $M/N = 1$, the trap equilibrium $\hat{\lambda} \approx 0.5$ remains empirically stable
with 100\% trap rate across 20 seeds (Table~\ref{tab:mn_sweep}).
The stabilizing mechanism has two components:
(i)~the sigmoid parametrization creates negative curvature
($\partial^2 V_i / \partial \theta_i^2 < 0$ at $\hat\theta = 0$),
making the equilibrium a local maximum of the per-agent value function in policy space;
(ii)~stochastic gradient noise creates a restoring drift toward the trap center.
\end{proposition}

\noindent\emph{Theoretical note}: a full Freidlin--Wentzell analysis of the noise-driven
landscape would establish the formal escape rate; we defer this to future work.
The above empirical result ($\bar\lambda = 0.503 \pm 0.010$, 100\% trap rate at $M/N{=}1.0$)
combined with the initialization sweep (Appendix~\ref{app:init_sweep}) provides strong evidence that
the trap is not an artifact of $M/N < 1$ incentive misalignment.

In both cases, escape requires coordinated perturbation of $\Omega(N)$ agents,
occurring with probability $e^{-\Omega(N)}$
under symmetric initialization.
