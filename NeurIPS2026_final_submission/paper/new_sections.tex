...
\emph{Case $M/N = 1$}: the myopic eigenvalue degenerates to zero, but the sigmoid parametrization
$\lambda_i = \sigma(\theta_i)$ introduces curvature: the Hessian in $\theta$-space at $\hat\theta = 0$
equals $-\operatorname{Var}[\nabla_{\theta_i} J_i] \cdot \sigma'(0)^2 < 0$, yielding stochastic stability
via the noise-driven restoring force (Appendix~\ref{app:impossibility_proof}, Lemma~\ref{lem:basin}).
...
(Lipschitz objective, bounded constraint set, Slater's condition),
...
\begin{proof}[Proof sketch]
This follows from convergence theory for projected subgradient methods
on saddle-point problems with convex constraints
(Nedic \& Ozdaglar, 2009).
The finite-difference gradient estimator introduces bias $O(\epsilon_{\mathrm{FD}}^2)$
and variance $O(\sigma^2 / n_{\mathrm{inner}})$,
which are controlled by choosing $\epsilon_{\mathrm{FD}} = O(K^{-1/4})$
and $n_{\mathrm{inner}} = O(\sqrt{K})$. \hfill $\square$
\end{proof}
