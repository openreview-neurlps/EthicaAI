...
\emph{Case $M/N = 1$}: the myopic eigenvalue degenerates to zero; we state the following:

\begin{conjecture}[Stochastic Stability at $M/N{=}1$]
\label{conj:mn1_stability}
When $M/N = 1$, $\hat{\lambda}$ remains stochastically stable
due to the curvature induced by sigmoid parametrization
($\partial^2 V_i / \partial \theta_i^2 < 0$ at $\hat\theta = 0$)
combined with stochastic gradient noise creating a restoring drift.
\end{conjecture}

\noindent\emph{Numerical evidence}: across 20 seeds at $M/N{=}1.0$,
$\bar\lambda = 0.503 \pm 0.010$ with 100\% trap rate (Table~\ref{tab:mn_sweep}).
Formal proof requires Freidlin--Wentzell analysis of the noise-driven
landscape, which we leave to future work; the $M/N < 1$ case is formally
proved above.
